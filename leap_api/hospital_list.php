...
//hospital details
 if((API_KEY == $_POST['API_KEY'] ) && isset($_POST['userid']) && isset($_POST['login_type']) ){
	...
	if($login_type == 1)		// Type-1 Hospital Doctors
	{
		...
		$getHospital = $objQuery->mysqlSelect("hosp_id, hosp_name, hosp_city, hosp_state","hosp_tab","","","","","0,20");
		} else {	
		$getHospital = $objQuery->mysqlSelect("hosp_id as hosp_id, hosp_name as hosp_name, hosp_city as hosp_city, hosp_state as hosp_state","hosp_tab","company_id='".$getInstituteType[0]['company_id']."'","hosp_name asc","","","0,50");
		}	
			if($getHospital == true)
				{
	...
	else if($login_type == 2)	// Type-2 Referring Partners
	{
		...
		$getHospital = $objQuery->mysqlSelect("hosp_id as hosp_id, hosp_name as hosp_name, hosp_city as hosp_city, hosp_state as hosp_state","hosp_tab","","hosp_name asc","","","0,20");
			if($getHospital == true)
				{
					$success = array('status' => "true","hospital_details" => $getHospital);
					echo json_encode($success);
	...
	{
		$getComapny = $objQuery->mysqlSelect("b.company_id as company_id","hosp_marketing_person as a inner join hosp_tab as b on b.hosp_id=a.hosp_id","a.person_id='".$user_id."'","","","","0,20");
		
		$getHospital = $objQuery->mysqlSelect("hosp_id, hosp_name, hosp_city, hosp_state","hosp_tab","company_id='".$getComapny[0]['company_id']."'","","","","0,20");
