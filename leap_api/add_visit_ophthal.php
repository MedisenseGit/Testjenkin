...
// Add Visit Ophthalmology
 if(API_KEY == $_POST['API_KEY']) {
	...
	if($login_type == 1) {						// Premium LoginType
			...
			$arrValuesPE[] = '2';						// EMR Type 1-Cardio and Genearl, 2- Ophthamology
		...
		if(!empty($_POST['prescription_ppID']))  {
			while (list($key, $val) = each($_POST['prescription_ppID'])) {
				...
				if(!empty($_POST['visit_entry_date'])) {
					...
					$arrValues[] = date('Y-m-d',strtotime($_POST['visit_entry_date']));
				}
				else {
					$arrFields[] = 'date_added';
					$arrValues[] =  date('Y-m-d',strtotime($Cur_Date)); ;
				}
				...
				$checkTrend= $objQuery->mysqlSelect("*","trend_analysis_ophthal","date_added='".date('Y-m-d',strtotime($_POST['visit_entry_date']))."' and patient_id='".$patient_id."' and patient_type='1'","","","","");
