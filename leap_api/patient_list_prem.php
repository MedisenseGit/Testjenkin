...
//Patient list
 if(API_KEY == $_POST['API_KEY'] && isset($_POST['userid']) && isset($_POST['login_type']) && isset($_POST['patient_filter']) ) {
	...
	else if($logintype == 3) //Login Type 3 for Marketing
	{
		...
		$getMapPatient = $objQuery->mysqlSelect("DISTINCT(b.patient_id) AS Pat_Id","patient_tab AS a INNER JOIN patient_referal AS b ON b.patient_id = a.patient_id INNER JOIN source_list AS c ON c.source_id = a.patient_src INNER JOIN doctor_hosp AS e ON e.doc_id = b.ref_id INNER JOIN mapping_hosp_referrer AS d ON c.partner_id = d.partner_id","d.market_person_id =  '".$marketingid."' AND e.hosp_id =  '".$getHospId[0]['hosp_id']."'","b.patient_id desc","","","0,20");
		if($getMapPatient==true){
			...
			if($patient_filter == 0) {
					...
					$getPatient = $objQuery->mysqlSelect('a.patient_id as pat_id, a.patient_name as pat_name, a.patient_age as pat_age, a.patient_loc as pat_loc, b.status2 as pat_status, c.ref_name as pat_doc_name, c.ref_id as pat_doc_id, d.source_name as pat_refered_by,b.timestamp as pat_status_time','patient_tab as a inner join patient_referal as b on a.patient_id = b.patient_id inner join referal as c on c.ref_id = b.ref_id inner join source_list as d on d.source_id = a.patient_src inner join doctor_hosp as e on e.doc_id=c.ref_id',"a.patient_id='".$patientList['Pat_Id']."' and b.status2 = '".$patient_filter."'","a.patient_id desc","","","0,20");
					$stuff['pat_id']=$getPatient[0]['pat_id'];
					$stuff['pat_name']=$getPatient[0]['pat_name'];
					$stuff['pat_age']=$getPatient[0]['pat_age'];
					$stuff['pat_loc']=$getPatient[0]['pat_loc'];
