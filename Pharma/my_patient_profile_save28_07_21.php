<?php ob_start();
	...
	if(isset($_POST['sendPayment']))
	{
		...
		if($_POST['type'] == '1'){
			...
			$link = HOST_MAIN_URL."Pharma/Patient-Profile-Payment?r=" . md5($_POST['referred_id'])."&t=1";//HOST_MAIN_URL."premium/Patient-Attachments?d=" . md5($getPatInfo[0]['patient_id']);
	
			$toEmail=$getPatInfo[0]['patient_email'];
		$mailSubject='Payment Link- Pharma';  
		$fromContent='Payment Link';
		$email='medical@medisense.me';
		$contentSection="Dear ".$getPatInfo[0]['patient_name'].",<br/> Please pay to get Prescription items. Please click the link to pay : ".$link." <br/>Thanks";
		...
		header("Location:Customer_Profile_Info?p=".md5($referred_id)."&response=".$response);	
			...
			$update_pay=mysqlUpdate('health_pharma_request',$arrFiedPay1,$arrValuePay1,"id='".$referred_id."'");
			
			$getPatInfo = mysqlSelect("*","health_pharma_request","id='".$referred_id."'" ,"","","","");
			$link = HOST_MAIN_URL."Pharma/Patient-Profile-Payment?r=" . md5($_POST['referred_id'])."&t=2";//HOST_MAIN_URL."premium/Patient-Attachments?d=" . md5($getPatInfo[0]['patient_id']);
	
			$toEmail=$getPatInfo[0]['customer_email'];
		$mailSubject='Payment Link- Pharma';  
		$fromContent='Payment Link';
		$email='medical@medisense.me';
		$contentSection="Dear ".$getPatInfo[0]['customer_name'].",<br/> Please pay to get Prescription items. Please click the link to pay : ".$link." <br/>Thanks";
	...
	if(isset($_POST['save_patient_edit']) || isset($_POST['save_patient_print'])){ //TO CHECK AUTHENTICATION OF POST VALUES
		//echo "<pre>"; print_r($_POST); exit;
			...
			//$txt_treatment =  $_POST['txt_treatment'];
				...
				$arrFieldsPE = array();
				$arrValuesPE = array();
				$arrFieldsPE[] = 'diagnostic_customer_id';
				$arrValuesPE[] = $patient_id;
				$arrFieldsPE[] = 'diagnostic_id';
			...
			$insert_temp_value=mysqlInsert('diagnostic_referrals',$arrFileds_referral,$arrValues_referral);
				...
				$arrValuesExam[] = "0";
				$update_icd=mysqlUpdate('diagnostic_patient_examination_active',$arrFieldsExam,$arrValuesExam,"diagnostic_customer_id = '".$patient_id."' and diagnostic_id='".$admin_id."' and status='1'");	
...
if(isset($_POST['addAttachments'])){
	//Save patient episode attachments
						...
						if(!empty($file_name)){
							...
							$bslist_pht=mysqlInsert('diagno_my_patient_reports',$arrFields_Attach,$arrValues_Attach);
