...
/*if(isset($_POST['cmdSearch'])){
	...
	$searchid = $params[0];
		...
		$allRecord = mysqlSelect("*","diagnostic_customer","diagnostic_id='".$admin_id."' and diagnostic_customer_id ='".$searchid."'","diagnostic_customer_id desc","diagnostic_customer_id","","$eu, $limit");
		$pag_result = mysqlSelect("diagnostic_customer_id","diagnostic_customer","diagnostic_id='".$admin_id."' and diagnostic_customer_id ='".$searchid."'","diagnostic_customer_id desc","diagnostic_customer_id");
		...
		$disp=1;
		header('Location:Customer_Profile_Info?p='.md5($allRecord[0]['diagnostic_customer_id']));	
		}
...
{*/
	...
	$TotalRecord = mysqlSelect("*","diagnostic_referrals","diagnostic_id='".$admin_id."'","dr_id desc","episode_id","","");	
	$total_rows = count($TotalRecord);
	$total_pages = ceil($total_rows / $no_of_records_per_page);
	
	$allRecord = mysqlSelect("*","diagnostic_referrals","diagnostic_id='".$admin_id."'","dr_id desc","","","$offset, $no_of_records_per_page");
	
	$TotalRecord1 = mysqlSelect("*","health_lab_test_request as a left join login_user as b on a.login_id=b.login_id","a.diagnostic_id='".$admin_id."'","a.id desc","","","");	
	$total_rows1 = count($TotalRecord1);
	$total_pages1 = ceil($total_rows1 / $no_of_records_per_page);
	$patientRecord = mysqlSelect("*","health_lab_test_request as a left join login_user as b on a.login_id=b.login_id","a.diagnostic_id='".$admin_id."'","a.id desc","","","$offset1, $no_of_records_per_page");

//$pag_result = mysqlSelect("a.dr_id","diagnostic_referrals as a left join diagnostic_customer as b on a.diagnostic_customer_id=b.diagnostic_customer_id","a.diagnostic_id='".$admin_id."'","a.diagnostic_customer_id desc","a.diagnostic_customer_id");
//$pageing = firstPaging($pag_result,$limit,$back,$next,$eu,$field,$type2);
//$arrPage = explode("-",$pageing);   
 $disp=1;
//}
                        ...
                        <table class="table table-striped">
                            ...
                            <tbody>
							 <?php foreach($allRecord as $list){ 
								 ...
								 $getPat = mysqlSelect("diagnostic_customer_name,diagnostic_customer_email,diagnostic_customer_phone","diagnostic_customer","diagnostic_customer_id='".$list['diagnostic_customer_id']."'","","","",""); 
							
								if($list['referred_by']== "1"){
									//$getRefByDet = mysqlSelect("sub_name","login_user","login_id='".$list['login_id']."'","","","",""); 
									 $refBy='<i class="fa fa-user"></i>  '.$getPat[0]['diagnostic_customer_name'];
								}
								else
								{
                            ...
                            <a href="Home"><tr>
                               <td><a href="Customer_Profile_Info?p=<?php echo md5($list['dr_id']); ?>"><?php echo $getPat[0]['diagnostic_customer_name'];  ?></a></td>
								 <td><?php echo date('M d, Y',strtotime($list['referred_date']));  ?></td>
                                <td><i class="fa fa-envelope"></i> <?php echo $getPat[0]['diagnostic_customer_email'];  ?><br>
											<i class="fa fa-mobile"></i> <?php echo $getPat[0]['diagnostic_customer_phone'];  ?></td>
