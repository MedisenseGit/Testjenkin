...
if($isValidChecksum == "TRUE") {
	...
	$getBookingTransactionDetails= mysqlSelect("*","payment_diag_pha_transaction","transaction_id='".$_POST["ORDERID"]."'","","","","");
	...
	if($_POST['STATUS']=="TXN_SUCCESS" && !empty($_POST['TXNID'])){
		...
		//Update Transaction booking table
			...
			$updateTransaction=mysqlUpdate('payment_diag_pha_transaction',$arrFields_trans,$arrValues_trans,"transaction_id='".$_POST['ORDERID']."'");
	...
	$getTransactionDetails= mysqlSelect("*","payment_diag_pha_transaction","transaction_id='".$_POST["ORDERID"]."'","","","","");
					...
					$diagno_referal = mysqlSelect("*","diagnostic_referrals","dr_id='".$getTransactionDetails[0]['referred_id']."'","","","","");					
					$checkPatient = mysqlSelect("*","diagnostic_customer","diagnostic_customer_id='".$diagno_referal[0]['diagnostic_customer_id']."'","","","",""); 
					
					$Name=$checkPatient[0]['diagnostic_customer_name'];
					$Email=$checkPatient[0]['diagnostic_customer_email'];
					$Mobile=$checkPatient[0]['diagnostic_customer_phone'];
					
					//$PatId=$getBookingDetails[0]['id'];
					if($getTransactionDetails[0]['type'] == '1'){
						$BookingId=$diagno_referal[0]['patient_id'];
					
					}
					else if($getTransactionDetails[0]['type'] == '2'){
						$BookingId=$diagno_referal[0]['login_id'];
					...
					$update_pay=mysqlUpdate('diagnostic_referrals',$arrFiedPay1,$arrValuePay1,"dr_id='".$getTransactionDetails[0]['referred_id']."'");
		...
		$mailSubject='Diagnostic Payment Details';  
		$fromContent='Diagnostic';
		$contentSection='Received a message from Diagnostic Page - Your Payment Transaction Details- <br/><br/> Contact Name: '.$Name.'<br/>Email : '.$Email.'  <br/> Mobile number: '.$Mobile.'  <br/> Transaction Id: '.$trans_id.'  <br/> Amount Paid: '.$trans_amount.'  <br/> Transaction Date: '.$trans_date.'<br/> Transaction Mode: '.$trans_mode.'';
...
<!-- Page Title -->
<title>Diagnostic Page - Payment Success</title>
