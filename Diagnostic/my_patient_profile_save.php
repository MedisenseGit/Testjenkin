<?php ob_start();
	...
	if(isset($_POST['sendPayment']))
	{
		...
		//if($_POST['type'] == '1'){
			...
			$update_pay=mysqlUpdate('diagnostic_referrals',$arrFiedPay1,$arrValuePay1,"dr_id='".$referred_id."'");
			
			//$getPatInfo = mysqlSelect("*","doc_my_patient","patient_id='".$_POST['patient_id']."'" ,"","","","");
			$diagno_referal = mysqlSelect("*","diagnostic_referrals","dr_id='".$_POST['referred_id']."'","","","","");
			 $getPatInfo = mysqlSelect("*","diagnostic_customer","diagnostic_customer_id='".$diagno_referal[0]['diagnostic_customer_id']."'","","","",""); 
			$link = HOST_MAIN_URL."Diagnostic/Patient-Profile-Payment?r=" . md5($_POST['referred_id'])."&t=".$_POST['type'];//HOST_MAIN_URL."premium/Patient-Attachments?d=" . md5($getPatInfo[0]['patient_id']);
	
			$toEmail=$getPatInfo[0]['diagnostic_customer_email'];
		$mailSubject='Payment Link- Diagnostic';  
		$fromContent='Payment Link';
		$email='medical@medisense.me';
		$contentSection="Dear ".$getPatInfo[0]['diagnostic_customer_name'].",<br/> Diagnostic Centre has sent payment link for investigations conducted. Please click the link to pay : ".$link." <br/>Thanks";
	...
	if(isset($_POST['save_patient_edit']) || isset($_POST['save_patient_print'])){ //TO CHECK AUTHENTICATION OF POST VALUES
		//echo "<pre>"; print_r($_POST); exit;
			...
			//$txt_treatment =  $_POST['txt_treatment'];
				...
				$episode_id = mysqli_insert_id(); //Get episode_id
...
if(isset($_POST['addAttachments'])){
	...
	//if($_POST['type'] == '1'){
			...
			$update_pay=mysqlUpdate('diagnostic_referrals',$arrFiedPay1,$arrValuePay1,"dr_id='".$_POST['referred_id']."'");
