...
if(API_KEY == $_POST['apikey']){
	...
	$txtPrefHosp = addslashes($_POST['patprefhosp']);
	$txtPrefDoc = addslashes($_POST['patprefdoc']);
	$AttachState = addslashes($_POST['attachState']);
	...
	if($ChkPatient==false){	
		...
		$userchat=$objQuery->mysqlInsert('chat_notification',$arrFields2,$arrValues2);
					...
					//SEND PATIENT_ID TO REMOTE SERVER
					if(!empty($TransId)){
					$url_page = 'getPatID.php';					
					$url = HOST_HEALTH_URL."CRM/";
					$url .= rawurlencode($url_page);
					$post .= "&patid=" . urlencode($id);
					$post .= "&transid=" . urlencode($TransId);
	...
	$url = "https://referralio.com/EMAIL/patmail.php?patmail=".$getPatInfo[0]['patient_email']."&patid=".$getPatInfo[0]['patient_id']."&patname=".$getPatInfo[0]['patient_name'];
		...
		//SMS notification to Successfull Registered Patient
					...
					$msg = urlencode ( "Hi ".$getPatInfo[0]['patient_name'].", We have registered your medical query with number: ". $getPatInfo[0]['patient_id'] . ", Kindly check your email for further details. Thanks.\n MedisenseHealth.com" );
		...
		$getDepartment = $objQuery->mysqlSelect("*","specialization","spec_id='".$getPatInfo[0]['medDept']."'" ,"","","","");
					...
					$tomail="medical@medisense.me".", ".$getPatInfo[0]['patient_email'];
