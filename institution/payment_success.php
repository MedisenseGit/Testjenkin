...
if($isValidChecksum == "TRUE") {
	...
	$getBookingTransactionDetails= mysqlSelect("*","emr_referred_transaction","transaction_id='".$_POST["ORDERID"]."'","","","","");
	...
	if($_POST['STATUS']=="TXN_SUCCESS" && !empty($_POST['TXNID'])){
		...
		//Update Transaction booking table
			...
			$updateTransaction=mysqlUpdate('emr_referred_transaction',$arrFields_trans,$arrValues_trans,"transaction_id='".$_POST['ORDERID']."'");
	...
	$getTransactionDetails= mysqlSelect("*","emr_referred_transaction","transaction_id='".$_POST["ORDERID"]."'","","","","");
	
	$checkPatient= mysqlSelect("a.patient_id as patient_id,a.patient_name as patient_name,a.patient_mobile as patient_mob,a.patient_email as patient_email","patients_appointment as a inner join patients_transactions as b on a.patient_id=b.patient_id","a.patient_id='".$getTransactionDetails[0]["patient_id"]."'","","","","");
	
	
	$getComp= mysqlSelect("*","compny_tab","company_id='".$getTransactionDetails[0]["company_id"]."'");
					...
					$ccmail="";
		if($getTransactionDetails[0]['type']=="1"){
			$typeName="Diagnosis";
			$type='1';
		}
		else if($getTransactionDetails[0]['type']=="2"){
			$typeName="Pharmacy";
			$type='2';
		}
		
		$chatMessage="Payment transaction successful";
				$arrFieldsChat = array();
				$arrValuesChat = array();
				$arrFieldsChat[] = 'patient_id';
				$arrValuesChat[] = $getTransactionDetails[0]["patient_id"];
				$arrFieldsChat[] = 'episode_id';
				$arrValuesChat[] = $getTransactionDetails[0]["episode_id"];				
				$arrFieldsChat[] = 'company_id';
				$arrValuesChat[] = $getTransactionDetails[0]["company_id"];
				...
				$arrValuesChat[] = $getTransactionDetails[0]['refer_id'];
				/*$arrFieldsChat[] = 'url';
		...
		$toEmail=$Email.','.$ccmail.','.$getComp[0]['email_id'];
		$mailSubject='EMR Referred to Institution Page Payment Details';  
		$fromContent='Medisense-Healthcare';
		$contentSection='Received a message from '.$typeName.' Referred Patient Page - Your Payment Transaction Details- <br/><br/> Contact Name: '.$Name.'<br/>Email : '.$Email.'  <br/> Mobile number: '.$Mobile.'  <br/> Transaction Id: '.$trans_id.'  <br/> Amount Paid: '.$trans_amount.'  <br/> Transaction Date: '.$trans_date.'<br/> Transaction Mode: '.$trans_mode.'';
...
<!-- Page Title -->
<title>EMR Referred Patient Page - Payment Success</title>
