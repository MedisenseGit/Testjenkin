...
require_once("../classes/querymaker.class.php");
			...
			$back = $eu - $limit; 
			$next = $eu + $limit;
$allRecord = mysqlSelect("DISTINCT(a.patient_id) as Patient_Id","patient_tab as a inner join patient_referal as b on a.patient_id=b.patient_id inner join doctor_hosp as c on c.doc_id=b.ref_id inner join hosp_tab as d on d.hosp_id=c.hosp_id","(b.status2=2 or b.status2=3) and (d.company_id='".$admin_id."')","b.timestamp desc","","","$eu, $limit");
$pag_result = mysqlSelect("DISTINCT(a.patient_id) as Patient_Id","patient_tab as a inner join patient_referal as b on a.patient_id=b.patient_id inner join doctor_hosp as c on c.doc_id=b.ref_id inner join hosp_tab as d on d.hosp_id=c.hosp_id","(b.status2=2 or b.status2=3) and (d.company_id='".$admin_id."')","b.timestamp desc");
