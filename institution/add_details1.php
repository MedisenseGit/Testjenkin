...
if(isset($_POST['updatePharmaPrescribe']))
{
			...
			else
			{
				...
				else if($getPatAttach==true || $getPatInfo[0]['repnotattach']==1 )
				{
					...
					if(!empty($getPatInfo[0]['patient_email']) && $get_pro[0]['communication_status']!=0)
					{
						...
						{   //IF SO, THEN SEND MEDI ASSIST LOGO
		...
		if(isset($_POST['add_doctor']))
		{
				...
				/* Uploading image file */ 
						...
						{ 
							...
							//INSERT DOCTOR TIME SLOT
							
							
				while(list($key_invest, $value_invest) = each($_POST['fromSun']))//ACADEMIC INFORMATION INSERT 
				{
	
					if(!empty($_POST['fromSun'][$key_invest]))
					{
						...
						$arrFields1[] = 'day_id';
						$arrValue1[]  = $_POST['Sunid'][$key_invest];
						
						$arrFields1[] = 'time_from';
						$arrValue1[]  = $_POST['fromSun'][$key_invest]; 
						
						$arrFields1[] = 'time_to';
						$arrValue1[]  = $_POST['toSun'][$key_invest]; 
						
						
							
						$doctimezonecreate = mysqlInsert('doctor_appointment_timezone_slots',$arrFields1,$arrValue1);
						
						$array_of_timeS = array ();
						$start_time    = strtotime ($_POST['fromSun'][$key_invest]);
						$end_time      = strtotime ($_POST['toSun'][$key_invest]);
	
						$fifteen_mins  = 15 * 60;
						while ($start_time <= $end_time)
						{
							$array_of_timeS[] = date ("H:i", $start_time);
							$start_time += $fifteen_mins;
						}
						
						
						$sun_slots="";	
						foreach ($array_of_timeS as $time_slotS) 					// utc_slots
						{
								...
								if($sun_slots!= "")
								$sun_slots .= ",";
								$sun_slots .= "'".$dateTimeUTC."'";
								
						}
						$get_timeid_sun=mysqlSelect("*","appointment_utc_slots","utc_slots in(".$sun_slots.")","","","","");
						
						foreach ($get_timeid_sun as $timeid_sun) 					// utc_slots
						{
								
								$arrFiedInvest_sun 	  = array();
								$arrValueInvest_sun   = array();
								
								$arrFiedInvest_sun[]  =  'time_id';
								$arrValueInvest_sun[] = $timeid_sun['id'];
								
								$arrFiedInvest_sun[]  =  'day_id';
								$arrValueInvest_sun[] =  $_POST['Sunid'][$key_invest];
								
								$arrFiedInvest_sun[]  =  'doc_id';
								$arrValueInvest_sun[] =   $id;
								
								$arrFiedInvest_sun[]  =  'hosp_id';
								$arrValueInvest_sun[] =   $slctHosp;
								
								$docslotcreate	=	mysqlInsert('doctor_appointment_slots_set',$arrFiedInvest_sun,$arrValueInvest_sun);
			...
			while(list($key_invest, $value_invest) = each($_POST['fromMon']))//ACADEMIC INFORMATION INSERT 
			{
	
				if(!empty($_POST['fromMon'][$key_invest]))
				{
					...
					$arrFieldsMon[] = 'day_id';
					$arrValueMon[]  = $_POST['Monid'][$key_invest];
					
					$arrFieldsMon[] = 'time_from';
					$arrValueMon[]  = $_POST['fromMon'][$key_invest]; 
					
					$arrFieldsMon[] = 'time_to';
					$arrValueMon[]  = $_POST['toMon'][$key_invest]; 
					
					$doctimezonecreate = mysqlInsert('doctor_appointment_timezone_slots',$arrFieldsMon,$arrValueMon);
					
					$array_of_timeM = array ();
					$start_time     = strtotime($_POST['fromMon'][$key_invest]);
					$end_time       = strtotime($_POST['toMon'][$key_invest]);
	
					
					$fifteen_mins  = 15 * 60;
					while ($start_time <= $end_time)
					{
						$array_of_timeM[] = date ("H:i", $start_time);
						$start_time += $fifteen_mins;
					}
					$Mon_slots="";		
					foreach ($array_of_timeM as $time_slotM) 					// utc_slots
					{
							$dateTime = $time_slotM; 
							$tz_from = $timezone; 
							$newDateTime = new DateTime($dateTime, new DateTimeZone($tz_from)); 
							$newDateTime->setTimezone(new DateTimeZone("UTC")); 
							$dateTimeUTC_M = $newDateTime->format("h:i a");
							
							if($Mon_slots!= "")
							$Mon_slots .= ",";
							$Mon_slots .= "'".$dateTimeUTC_M."'";
					}
					$get_timeid_Mon=mysqlSelect("*","appointment_utc_slots","utc_slots in(".$Mon_slots.")","","","","");
										
					foreach ($get_timeid_Mon as $timeid_Mon) 					// utc_slots
					{
							
						$arrFiedInvest_Mon 	  = array();
						$arrValueInvest_Mon   = array();
						
						$arrFiedInvest_Mon[]  =  'time_id';
						$arrValueInvest_Mon[] = $timeid_Mon['id'];
						
						$arrFiedInvest_Mon[]  =  'day_id';
						$arrValueInvest_Mon[] =  $_POST['Monid'][$key_invest];
						
						$arrFiedInvest_Mon[]  =  'doc_id';
						$arrValueInvest_Mon[] =   $id;
						
						$arrFiedInvest_Mon[]  =  'hosp_id';
						$arrValueInvest_Mon[] =   $slctHosp;
						
						$docslotcreate=mysqlInsert('doctor_appointment_slots_set',$arrFiedInvest_Mon,$arrValueInvest_Mon);
	...
	// slot for tuesday 				
			while(list($key_invest, $value_invest) = each($_POST['fromTue']))//ACADEMIC INFORMATION INSERT 
			{
				if(!empty($_POST['fromTue'][$key_invest]))
				{
					...
					$arrFieldsTue[] = 'day_id';
					$arrValueTue[]  = $_POST['Tueid'][$key_invest];
					
					$arrFieldsTue[] = 'time_from';
					$arrValueTue[]  = $_POST['fromTue'][$key_invest];  
					
					$arrFieldsTue[] = 'time_to';
					$arrValueTue[]  = $_POST['toTue'][$key_invest];
					
					
					$doctimezonecreate = mysqlInsert('doctor_appointment_timezone_slots',$arrFieldsTue,$arrValueTue);
					
					$array_of_timeTU = array ();
					$start_time    = strtotime ($_POST['fromTue'][$key_invest]);
					$end_time      = strtotime ($_POST['toTue'][$key_invest]);
	
					$fifteen_mins  = 15 * 60;
					while ($start_time <= $end_time)
					{
						$array_of_timeTU[] = date ("H:i", $start_time);
						$start_time += $fifteen_mins;
					}
						
					
					$Tue_slots="";	
					foreach ($array_of_timeTU as $time_slotTU) 					// utc_slots
					{
							$dateTime 	= $time_slotTU; 
							$tz_from 	= $timezone; 
							$newDateTime = new DateTime($dateTime, new DateTimeZone($tz_from)); 
							$newDateTime->setTimezone(new DateTimeZone("UTC")); 
							$dateTimeUTC_T = $newDateTime->format("h:i a");
							
							if($Tue_slots!= "")
							$Tue_slots .= ",";
							$Tue_slots .= "'".$dateTimeUTC_T."'";
					}
					$get_timeid_Tue=mysqlSelect("*","appointment_utc_slots","utc_slots in(".$Tue_slots.")","","","","");
					foreach ($get_timeid_Tue as $timeid_Tue) 					// utc_slots
					{
							
							$arrFiedInvest_Tue 	  = array();
							$arrValueInvest_Tue   = array();
							
							$arrFiedInvest_Tue[]  =  'time_id';
							$arrValueInvest_Tue[] = $timeid_Tue['id'];
							
							$arrFiedInvest_Tue[]  =  'day_id';
							$arrValueInvest_Tue[] =  $_POST['Tueid'][$key_invest];
							
							$arrFiedInvest_Tue[]  =  'doc_id';
							$arrValueInvest_Tue[] =   $id;
							
							$arrFiedInvest_Tue[]  =  'hosp_id';
							$arrValueInvest_Tue[] =   $slctHosp;
							
							
							$docslotcreate=mysqlInsert('doctor_appointment_slots_set',$arrFiedInvest_Tue,$arrValueInvest_Tue);
					}
				}	
			}
	
		
			while(list($key_invest, $value_invest) = each($_POST['fromWed']))//ACADEMIC INFORMATION INSERT 
			{
				if(!empty($_POST['fromWed'][$key_invest]))
				{
					$arrFieldsWed = array();
					$arrValueWed  = array();
					...
					$arrFieldsWed[] = 'day_id';
					$arrValueWed[]  = $_POST['Wedid'][$key_invest];
					
					$arrFieldsWed[] = 'time_from';
					$arrValueWed[]  = $_POST['fromWed'][$key_invest]; 
					
					$arrFieldsWed[] = 'time_to';
					$arrValueWed[]  = $_POST['toWed'][$key_invest]; 
					
					
					$doctimezonecreate = mysqlInsert('doctor_appointment_timezone_slots',$arrFieldsWed,$arrValueWed);
					
					
					$array_of_timeWed = array ();
					$start_time    = strtotime ($_POST['fromWed'][$key_invest]);
					$end_time      = strtotime ($_POST['toWed'][$key_invest]);
	
					$fifteen_mins  = 15 * 60;
					while ($start_time <= $end_time)
					{
						$array_of_timeWed[] = date ("H:i", $start_time);
						$start_time += $fifteen_mins;
					}
						
					
					$Wed_slots="";	
					foreach ($array_of_timeWed as $time_slotWed) 					// utc_slots
					{
							
						$dateTime 	= $time_slotWed; 
						$tz_from 	= $timezone; 
						$newDateTime = new DateTime($dateTime, new DateTimeZone($tz_from)); 
						$newDateTime->setTimezone(new DateTimeZone("UTC")); 
						$dateTimeUTC_W = $newDateTime->format("h:i a");
						
						if($Wed_slots!= "")
						$Wed_slots .= ",";
						$Wed_slots .= "'".$dateTimeUTC_W."'";
								
					}
					$get_timeid_Wed=mysqlSelect("*","appointment_utc_slots","utc_slots in(".$Wed_slots.")","","","","");
					foreach ($get_timeid_Wed as $timeid_Wed) 					// utc_slots
					{
							
						$arrFiedInvest_Wed 	  = array();
						$arrValueInvest_Wed   = array();
						
						$arrFiedInvest_Wed[]  =  'time_id';
						$arrValueInvest_Wed[] = $timeid_Wed['id'];
						
						$arrFiedInvest_Wed[]  =  'day_id';
						$arrValueInvest_Wed[] =  $_POST['Wedid'][$key_invest];
						
						$arrFiedInvest_Wed[]  =  'doc_id';
						$arrValueInvest_Wed[] =   $id;
						
						$arrFiedInvest_Wed[]  =  'hosp_id';
						$arrValueInvest_Wed[] =   $slctHosp;
						
						
						$docslotcreate=mysqlInsert('doctor_appointment_slots_set',$arrFiedInvest_Wed,$arrValueInvest_Wed);
					}
				}	
			}
			//SLOT FOR THURSDAY 
			...
			while(list($key_invest, $value_invest) = each($_POST['fromThu']))//ACADEMIC INFORMATION INSERT 
			{
				if(!empty($_POST['fromThu'][$key_invest]))
				{
					$arrFieldsThu = array();
					$arrValueThu  = array();
					...
					$arrFieldsThu[] = 'day_id';
					$arrValueThu[]  = $_POST['Thuid'][$key_invest];
					
					$arrFieldsThu[] = 'time_from';
					$arrValueThu[]  = $_POST['fromThu'][$key_invest];
					
					$arrFieldsThu[] = 'time_to';
					$arrValueThu[]  = $_POST['toThu'][$key_invest]; 
					
					
					$doctimezonecreate = mysqlInsert('doctor_appointment_timezone_slots',$arrFieldsThu,$arrValueThu);
				
					
					$array_of_timeThu = array ();
					$start_time    = strtotime ($_POST['fromThu'][$key_invest]);
					$end_time      = strtotime ($_POST['toThu'][$key_invest]);
	
					$fifteen_mins  = 15 * 60;
					while ($start_time <= $end_time)
					{
						$array_of_timeThu[] = date ("H:i", $start_time);
						$start_time += $fifteen_mins;
					}
					
					$Thu_slots="";	
					foreach ($array_of_timeThu as $time_slotThu) 					// utc_slots
					{
							
						$dateTime = $time_slotThu; 
						$tz_from = $timezone; 
						$newDateTime = new DateTime($dateTime, new DateTimeZone($tz_from)); 
						$newDateTime->setTimezone(new DateTimeZone("UTC")); 
						$dateTimeUTC_Th = $newDateTime->format("h:i a");
						
						if($Thu_slots!= "")
						$Thu_slots .= ",";
						$Thu_slots .= "'".$dateTimeUTC_Th."'";
								
					}
					$get_timeid_Thu = mysqlSelect("*","appointment_utc_slots","utc_slots in(".$Thu_slots.")","","","","");
					foreach ($get_timeid_Thu as $timeid_Thu) 					// utc_slots
					{
							
						$arrFiedInvest_Thu 	  = array();
						$arrValueInvest_Thu   = array();
						
						$arrFiedInvest_Thu[]  =  'time_id';
						$arrValueInvest_Thu[] =  $timeid_Thu['id'];
						
						$arrFiedInvest_Thu[]  =  'day_id';
						$arrValueInvest_Thu[] =  $_POST['Thuid'][$key_invest];
						
						$arrFiedInvest_Thu[]  =  'doc_id';
						$arrValueInvest_Thu[] =   $id;
						
						$arrFiedInvest_Thu[]  =  'hosp_id';
						$arrValueInvest_Thu[] =   $slctHosp;
						
						$docslotcreate=mysqlInsert('doctor_appointment_slots_set',$arrFiedInvest_Thu,$arrValueInvest_Thu);
					}
				}
			}
	
							
			while(list($key_invest, $value_invest) = each($_POST['fromFri']))//ACADEMIC INFORMATION INSERT 
			{
				if(!empty($_POST['fromFri'][$key_invest]))
				{
					...
					$arrFieldsFri[] = 'day_id';
					$arrValueFri[]  = $_POST['Friid'][$key_invest];
					
					$arrFieldsFri[] = 'time_from';
					$arrValueFri[]  = $_POST['fromFri'][$key_invest]; 
					
					$arrFieldsFri[] = 'time_to';
					$arrValueFri[]  = $_POST['toFri'][$key_invest]; 
					
					$doctimezonecreate = mysqlInsert('doctor_appointment_timezone_slots',$arrFieldsFri,$arrValueFri);
					
					$array_of_timeFri = array ();
					$start_time    = strtotime ($_POST['fromFri'][$key_invest]);
					$end_time      = strtotime ($_POST['toFri'][$key_invest]);
	
					$fifteen_mins  = 15 * 60;
					while ($start_time <= $end_time)
					{
						$array_of_timeFri[] = date ("H:i", $start_time);
						$start_time += $fifteen_mins;
					}
					
					$Fri_slots="";	
					foreach ($array_of_timeFri as $time_slotFri) 					// utc_slots
					{
							
						$dateTime = $time_slotFri; 
						$tz_from = $timezone; 
						$newDateTime = new DateTime($dateTime, new DateTimeZone($tz_from)); 
						$newDateTime->setTimezone(new DateTimeZone("UTC")); 
						$dateTimeUTC_Fri = $newDateTime->format("h:i a");
							
						if($Fri_slots!= "")
						$Fri_slots .= ",";
						$Fri_slots .= "'".$dateTimeUTC_Fri."'";
							
					}
					$get_timeid_Fri = mysqlSelect("*","appointment_utc_slots","utc_slots in(".$Fri_slots.")","","","","");
					foreach ($get_timeid_Fri as $timeid_Fri) 					// utc_slots
					{
							
						$arrFiedInvest_Fri 	  = array();
						$arrValueInvest_Fri   = array();
						
						$arrFiedInvest_Fri[]  =  'time_id';
						$arrValueInvest_Fri[] = $timeid_Fri['id'];
						
						$arrFiedInvest_Fri[]  =  'day_id';
						$arrValueInvest_Fri[] =  $_POST['Friid'][$key_invest];
						
						$arrFiedInvest_Fri[]  =  'doc_id';
						$arrValueInvest_Fri[] =   $id;
						
						$arrFiedInvest_Fri[]  =  'hosp_id';
						$arrValueInvest_Fri[] =   $slctHosp;
						
						$docslotcreate=mysqlInsert('doctor_appointment_slots_set',$arrFiedInvest_Fri,$arrValueInvest_Fri);
					}
				}
			}
			// SLOT FOR SATURDAY 
								
			while(list($key_invest, $value_invest) = each($_POST['fromSat']))//ACADEMIC INFORMATION INSERT 
			{
				
				if(!empty($_POST['fromSat'][$key_invest]))
				{
					...
					$arrFieldsSat[] = 'day_id';
					$arrValueSat[]  = $_POST['Satid'][$key_invest];
					
					$arrFieldsSat[] = 'time_from';
					$arrValueSat[]  = $_POST['fromSat'][$key_invest]; 
					
					$arrFieldsSat[] = 'time_to';
					$arrValueSat[]  = $_POST['toSat'][$key_invest]; 	
					
					$doctimezonecreate = mysqlInsert('doctor_appointment_timezone_slots',$arrFieldsSat,$arrValueSat);
					
					$array_of_timeSat 	= array ();
					$start_time    		= strtotime ($_POST['fromSat'][$key_invest]);
					$end_time      		= strtotime ($_POST['toSat'][$key_invest]);
	
					$fifteen_mins  = 15 * 60;
					while ($start_time <= $end_time)
					{
						$array_of_timeSat[] = date ("H:i", $start_time);
						$start_time += 	$fifteen_mins;
					}
					
					$Sat_slots	=	"";	
					foreach ($array_of_timeSat as $time_slotSat) 					// utc_slots
					{
							
						$dateTime = $time_slotSat; 
						$tz_from  = $timezone; 
						$newDateTime = new DateTime($dateTime, new DateTimeZone($tz_from)); 
						$newDateTime->setTimezone(new DateTimeZone("UTC")); 
						$dateTimeUTC_Sat = $newDateTime->format("h:i a");
							
						if($Sat_slots!= "")
						$Sat_slots .= ",";
						$Sat_slots .= "'".$dateTimeUTC_Sat."'";
							
					}
					
					$get_timeid_Sat = mysqlSelect("*","appointment_utc_slots","utc_slots in(".$Sat_slots.")","","","","");
					foreach ($get_timeid_Sat as $timeid_Sat) 					// utc_slots
					{
							
						$arrFiedInvest_Sat 	  = array();
						$arrValueInvest_Sat  = array();
						
						$arrFiedInvest_Sat[]  =  'time_id';
						$arrValueInvest_Sat[] = $timeid_Sat['id'];
						
						$arrFiedInvest_Sat[]  =  'day_id';
						$arrValueInvest_Sat[] =  $_POST['Satid'][$key_invest];
						
						$arrFiedInvest_Sat[]  =  'doc_id';
						$arrValueInvest_Sat[] =   $id;
						
						$arrFiedInvest_Sat[]  =  'hosp_id';
						$arrValueInvest_Sat[] =   $slctHosp;
						
						
						$docslotcreate=mysqlInsert('doctor_appointment_slots_set',$arrFiedInvest_Sat,$arrValueInvest_Sat);
					}
				}	
			}
											
			
			header("Location:Add-Hospital-Doctors1?response=add");
		}
	...
	{	
				...
				else
				{
					...
					$usercraete=mysqlInsert('doctor_hosp',$arrFields1,$arrValues1);
						...
						$delect_doc_slot =  mysqlDelete('doctor_appointment_timezone_slots',"doc_id ='".$_POST['Prov_Id']."'  and  day_id ='".$_POST['Sunid'][0]."'");
						$delect_doc_slot =  mysqlDelete('doctor_appointment_slots_set',"doc_id ='".$_POST['Prov_Id']."'  and  day_id ='".$_POST['Sunid'][0]."'");
						...
						while(list($key_invest, $value_invest) = each($_POST['fromSun']))//ACADEMIC INFORMATION INSERT 
						{
							if(!empty($_POST['fromSun'][$key_invest]))
							{
								...
								foreach ($array_of_timeS as $time_slotS) 					// utc_slots
								{
									...
									$newDateTime = new DateTime($dateTime, new DateTimeZone($tz_from)); 
									$newDateTime->setTimezone(new DateTimeZone("UTC")); 
									$dateTimeUTC_S = $newDateTime->format("h:i a");
									
									if($sun_slots!= "")
									$sun_slots .= ",";
									$sun_slots .= "'".$dateTimeUTC_S."'";
						...
						$delect_doc_slotM =  mysqlDelete('doctor_appointment_timezone_slots',"doc_id ='".$_POST['Prov_Id']."'  and  day_id ='".$_POST['Monid'][0]."'");
						$delect_doc_slotM =  mysqlDelete('doctor_appointment_slots_set',"doc_id ='".$_POST['Prov_Id']."'  and  day_id ='".$_POST['Monid'][0]."'");
						...
						while(list($key_invest, $value_invest) = each($_POST['fromMon']))//ACADEMIC INFORMATION INSERT 
						{
							if(!empty($_POST['fromMon'][$key_invest]))
							{
								...
								foreach ($get_timeid_Mon as $timeid_Mon) 					// utc_slots
								{
									...
									$arrFiedInvest_Mon[]  =  'doc_id';
									$arrValueInvest_Mon[] =   $_POST['Prov_Id'];
						...
						$delect_doc_slotT =  mysqlDelete('doctor_appointment_timezone_slots',"doc_id ='".$_POST['Prov_Id']."'  and  day_id ='".$_POST['Tueid'][0]."'");
						$delect_doc_slotT =  mysqlDelete('doctor_appointment_slots_set',"doc_id ='".$_POST['Prov_Id']."'  and  day_id ='".$_POST['Tueid'][0]."'");
						...
						while(list($key_invest, $value_invest) = each($_POST['fromTue']))//ACADEMIC INFORMATION INSERT 
						{
							if(!empty($_POST['fromTue'][$key_invest]))
							{
								...
								foreach ($array_of_timeTU as $time_slotTU) 					// utc_slots
								{
									...
									$newDateTime = new DateTime($dateTime, new DateTimeZone($tz_from)); 
									$newDateTime->setTimezone(new DateTimeZone("UTC")); 
									$dateTimeUTC_TU = $newDateTime->format("h:i a");
									
									if($Tue_slots!= "")
									$Tue_slots .= ",";
									$Tue_slots .= "'".$dateTimeUTC_TU."'";
								...
								foreach ($get_timeid_Tue as $timeid_Tue) 					// utc_slots
								{
										...
										$arrFiedInvest_Tue[]  =  'doc_id';
										$arrValueInvest_Tue[] =   $_POST['Prov_Id'];
						...
						$delect_doc_slotW =  mysqlDelete('doctor_appointment_timezone_slots',"doc_id ='".$_POST['Prov_Id']."'  and  day_id ='".$_POST['Wedid'][0]."'");
						$delect_doc_slotW =  mysqlDelete('doctor_appointment_slots_set',"doc_id ='".$_POST['Prov_Id']."'  and  day_id ='".$_POST['Wedid'][0]."'");	
						while(list($key_invest, $value_invest) = each($_POST['fromWed']))//ACADEMIC INFORMATION INSERT 
						{
							if(!empty($_POST['fromWed'][$key_invest]))
							{
						...
						$delect_doc_slotTh =  mysqlDelete('doctor_appointment_timezone_slots',"doc_id ='".$_POST['Prov_Id']."'  and  day_id ='".$_POST['Thuid'][0]."'");
						$delect_doc_slotTh =  mysqlDelete('doctor_appointment_slots_set',"doc_id ='".$_POST['Prov_Id']."'  and  day_id ='".$_POST['Thuid'][0]."'");	
						...
						while(list($key_invest, $value_invest) = each($_POST['fromThu']))//ACADEMIC INFORMATION INSERT 
						{
							if(!empty($_POST['fromThu'][$key_invest]))
							{
								...
								foreach ($array_of_timeThu as $time_slotThu) 					// utc_slots
								{
									...
									$newDateTime = new DateTime($dateTime, new DateTimeZone($tz_from)); 
									$newDateTime->setTimezone(new DateTimeZone("UTC")); 
									$dateTimeUTC_Thu = $newDateTime->format("h:i a");
										
									if($Thu_slots!= "")
									$Thu_slots .= ",";
									$Thu_slots .= "'".$dateTimeUTC_Thu."'";
								...
								foreach ($get_timeid_Thu as $timeid_Thu) 					// utc_slots
								{
									...
									$arrFiedInvest_Thu[]  =  'doc_id';
									$arrValueInvest_Thu[] =   $_POST['Prov_Id'];
						...
						$delect_doc_slotF =  mysqlDelete('doctor_appointment_timezone_slots',"doc_id ='".$_POST['Prov_Id']."'  and  day_id ='".$_POST['Friid'][0]."'");
						$delect_doc_slotF =  mysqlDelete('doctor_appointment_slots_set',"doc_id ='".$_POST['Prov_Id']."'  and  day_id ='".$_POST['Friid'][0]."'");
						...
						while(list($key_invest, $value_invest) = each($_POST['fromFri']))//ACADEMIC INFORMATION INSERT 
						{
							if(!empty($_POST['fromFri'][$key_invest]))
							{
								...
								foreach ($get_timeid_Fri as $timeid_Fri) 					// utc_slots
								{
										...
										$arrFiedInvest_Fri[]  =  'doc_id';
										$arrValueInvest_Fri[] =   $_POST['Prov_Id'];
						...
						$delect_doc_slotS =  mysqlDelete('doctor_appointment_timezone_slots',"doc_id ='".$_POST['Prov_Id']."'  and  day_id ='".$_POST['Satid'][0]."'");
						$delect_doc_slotS =  mysqlDelete('doctor_appointment_slots_set',"doc_id ='".$_POST['Prov_Id']."'  and  day_id ='".$_POST['Satid'][0]."'");
						...
						while(list($key_invest, $value_invest) = each($_POST['fromSat']))//ACADEMIC INFORMATION INSERT 
						{
							...
							if(!empty($_POST['fromSat'][$key_invest]))
							{
								...
								foreach ($get_timeid_Sat as $timeid_Sat) 					// utc_slots
								{
									...
									$arrFiedInvest_Sat[]  =  'doc_id';
									$arrValueInvest_Sat[] =   $_POST['Prov_Id'];
						...
						header("Location:Add-Hospital-Doctors1?response=update");
			}
		}	
		//Send Error message
		else
		{
			header("Location:Add-Hospital-Doctors1?response=error");
		}
	}
	...
	if(isset($_POST['ref_appointment'])){
		...
		$chkInDate = date('Y-m-d'); //Current Date
			...
			$GetTiming= mysqlSelect("*","seven_days as a left join doc_time_set as b on a.day_id=b.day_id","b.doc_id='".$_POST['se_doc']."' and b.hosp_id='".$_POST['se_hosp']."' and a.da_name='".$day_val."'","b.time_id desc","","","");
			foreach($GetTiming as $TimeList) {
			$chkDocTimeSlot = mysqlSelect("num_patient_hour","doc_appointment_slots","doc_id='".$_POST['se_doc']."' and doc_type='1' and hosp_id = '".$_POST['se_hosp']."'","","","","");
			
			//echo $chkDocTimeSlot[0]['num_patient_hour'];
			$countPrevAppBook = mysqlSelect("COUNT(id) as Appoint_Count","appointment_transaction_detail","pref_doc='".$_POST['se_doc']."' and hosp_id = '".$_POST['se_hosp']."' and Visiting_date = '".$chkInDate."' and Visiting_time = '".$TimeList["time_id"]."'","","","","");
