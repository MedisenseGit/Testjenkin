...
if(isset($_POST['appointmentUpcoming'])){	
		...
		$getRefDet = mysqlSelect("doc_state,ref_address,cons_charge","referal","","","","","");             
...
<body onLoad="set_interval(); document.form1.exp_dat.focus();">
					 ...
					 <table class="footable table table-stripped" data-page-size="100" data-filter=#filter>
						...
						<tbody>
							...
							foreach($appointmentToday as $Todaylist){ 
								...
								$getDoc= mysqlSelect("ref_name","referal","ref_id='".$Todaylist['doc_id']."'","","","","");						
							?>
							<script>
							...
							<tr>
								...
								<td><?php echo $Todaylist['appoint_trans_id']; ?></td>
								<td><a href="<?php echo $_SESSION['EMR_URL'].md5($Todaylist['patient_id']);?>"><?php echo $Todaylist['patient_name']; ?></a></td>
								<td style="min-width:150px;" ><?php echo $getDoc[0]['ref_name']."<br>".date('d-m-Y',strtotime($Todaylist['app_date']))." | ".$Todaylist['app_time']; ?></td>
								<td>
								<div class="btn-group pull-right">
								<?php 
								...
								<form method="post" enctype="multipart/form-data" class="form-horizontal form-label-left"  action="add_details.php"  name="frmAddPatient" id="frmAddPatient">
									...
									$chkdate=date('D', $date);
									$getDocDays= mysqlSelect("DISTINCT(b.day_id) as DayId","doc_time_set as a left join seven_days as b on a.day_id=b.day_id","a.doc_id='".$admin_id."' and a.hosp_id='".$admin_id."'","","","","");
						...
						<tbody>
							...
							<?php $getDoctorName= mysqlSelect("ref_id as Ref_Id,ref_name as Doc_name","referal","ref_id='".$list['doc_id']."'","ref_name asc","","","");
								...
								<td style="min-width:150px;" ><?php echo $getDoctorName[0]['Doc_name']."<br>".date('d-m-Y',strtotime($list['app_date']))." | ".$list['app_time']; ?></td>
								<td>
								<div class="btn-group pull-right">
								<?php 
						...
						<tr>
							...
							$getTimeSlot= mysqlSelect("*","timings","Timing_id='".$Alllist['Visit_Time']."'","","","","");
							...
							function getDocTiming<?php echo $Alllist['appoint_trans_id']; ?>(val) {
								...
								data:'day_val='+val,
								success: function(data){
									$("#check_time<?php echo $Alllist['Trans_ID']; ?>").html(data);
								}
								});
							}
							</script>	
							...
							<?php $getDoctorName= mysqlSelect("ref_id as Ref_Id,ref_name as Doc_name","referal","ref_id='".$Alllist['Pref_Doc']."'","ref_name asc","","","");
												?>
								 <td><?php echo $Alllist['Trans_ID']; ?></td>
								<td><a href="<?php echo $_SESSION['EMR_URL']."?p=".md5($Alllist['patient_id']);?>"><?php echo $Alllist['Patient_name']; ?></a></td>
								<td style="min-width:150px;" ><?php echo $getDoctorName[0]['Doc_name'] ."<br>". date('d-m-Y',strtotime($Alllist['Visit_Date']))." | ".$getTimeSlot[0]['Timing']; ?></td>
								<td>
								<div class="btn-group pull-right">
								<?php 
								...
								?>
									  ...
									  <?php foreach($status_val as $key=>$value){ ?>
									   <li><a href="#" class="patient-status" data-status-id="<?php echo $value; ?>" data-appoint-transid="<?php echo $Alllist['Trans_ID']; ?>"><?php echo $key; ?></a></li>
									  <?php } ?>
										</ul>
										 <?php } ?>
								</td>
								<td><center>
									<?php if($Alllist['Pay_Status']!="Cancelled"){ ?>	<a href="Reschedule?a=<?php echo $Alllist['Trans_ID']; ?>"><i class="fa fa-edit"></i></a> </center><?php } ?>
										
										
										<div class="modal inmodal" id="myModal<?php echo $Alllist['Trans_ID']; ?>" tabindex="-1" role="dialog" aria-hidden="true">
                                <div class="modal-dialog">
                                <div class="modal-content animated bounceInRight">
								<form method="post" enctype="multipart/form-data" class="form-horizontal form-label-left"  action="add_details.php"  name="frmAddPatient" id="frmAddPatient">
										
                                        
										<input type="hidden" name="trans_id" value="<?php echo $Alllist['Trans_ID']; ?>">
                                    <div class="modal-header">
                                            <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
                                    ...
                                    <div class="col-sm-4"><select data-placeholder="Choose Preferred Time..." class="form-control chkTime" name="check_time"  id="check_time<?php echo $Alllist['Trans_ID']; ?>" tabindex="2" required="required">
			...
			<div class="tabs-container">
                        ...
                        <div class="tab-content">
                            <div id="tab-1" class="tab-pane active">
								...
								//echo "<PRE>"; print_r($_SESSION); exit;
								$chkDocTimeSlot= mysqlSelect("*","seven_days as a left join doc_time_set as b on a.day_id=b.day_id"," b.hosp_id='".$admin_id."' and a.da_name='".$day_val."'","","","","");
								
								$docNumPatientHour= mysqlSelect("num_patient_hour","doc_appointment_slots"," doc_type='1' and hosp_id='".$admin_id."'","","","","");
								$totalPatientToday = count($chkDocTimeSlot)*$docNumPatientHour[0]['num_patient_hour'];
		
								$curBookingCount= mysqlSelect("*","appointment_token_system","app_date='".$curdate."' and  doc_type='1' and hosp_id='".$admin_id."'","","","","");
							...
							<form enctype="multipart/form-data" method="post" action="add_details.php"  name="frmAddPatient" autocomplete="off" >
									...
									<!--<div class="form-group" style="padding-top: 30px;">
										...
										<div class="col-sm-10"><select data-placeholder="Choose doctor..." class="chosen-select" name="se_hosp" id="se_hosp" required="required"  tabindex="2" onchange="return getDocDet(this.value); ">
											<option value="" selected >Choose Hospital</option>
												...
												$hospResult = mysqlSelect("hosp_id as Hosp_Id,hosp_name as Hosp_Name,hosp_city as Hosp_City,hosp_state as Hosp_State","hosp_tab","","hosp_id desc","","","");
													$i=30;
									...
									$getDoctor	= mysqlSelect("a.ref_id as Ref_Id,a.ref_name as Doc_name,c.hosp_name as Hosp_name,d.spec_name as Department","referal as a inner join doctor_hosp as b on a.ref_id=b.doc_id inner join hosp_tab as c on c.hosp_id=b.hosp_id inner join doc_specialization as e on e.doc_id=a.ref_id inner join specialization as d on d.spec_id=e.spec_id","","a.ref_name desc","","","");
										...
										<div class="col-sm-10">
										<select data-placeholder="Choose doctor..." class="chosen-select" name="doc_id" id="doc_id" required="required"  tabindex="2" onchange="return getDocDate(this.value);">
												<option value="" selected>Choose Doctor</option>
													<?php 
													// $getDoctor= mysqlSelect("a.ref_id as Ref_Id,a.ref_name as Doc_name,c.hosp_name as Hosp_name,d.spec_name as Department","referal as a inner join doctor_hosp as b on a.ref_id=b.doc_id inner join hosp_tab as c on c.hosp_id=b.hosp_id inner join doc_specialization as e on e.doc_id=a.ref_id inner join specialization as d on d.spec_id=e.spec_id","a.company_id='".$admin_id."'","a.ref_name asc","","","");
                                    ...
                                    <div class="col-sm-4"><input type="text" name="se_pat_name" required="required" class="form-control" required="" value="<?php echo $get_PatientDetails[0]['patient_name']; ?>"></div>
  ...
  <!-- Mainly scripts -->
   ...
   <script src="../assets/js/plugins/datapicker/bootstrap-datepicker.js"></script>
	...
	$get_PatientDetails = mysqlSelect("e.patient_id,e.patient_name,e.patient_mob","doc_my_patient as e inner join referal as a on a.ref_id=e.doc_id inner join doctor_hosp as b on a.ref_id=b.doc_id inner join hosp_tab as c on c.hosp_id=b.hosp_id inner join specialization as d on d.spec_id=a.doc_spec","","","","","");
