...
$getSource = mysqlSelect("*","source_list","source_id='".$GetPatient[0]['patient_src']."'","","","","");
...
if(isset($_POST['cmdchangeStatus'])){
	...
	//GET Patient Details
	$getPatient = mysqlSelect("a.patient_name as PatName,a.patient_loc as Pat_loc,a.patient_id as Pat_Id,b.status2 as Current_Status,a.patient_src as Source_id","patient_tab as a left join patient_referal as b on a.patient_id=b.patient_id","a.patient_id='".$_POST['patient_id']."'and b.ref_id='".$_POST['doc_id']."'","","","","");
...
$get_pro = mysqlSelect("a.ref_id as RefId","referal as a inner join doctor_hosp as b on a.ref_id=b.doc_id inner join hosp_tab as c on c.hosp_id=b.hosp_id","c.company_id='".$admin_id."'","a.Tot_responded desc","","","");
...
<body>
    ...
    <?php include_once('sidemenu.php'); ?>
        ...
        <div class="row">
                            ...
                            <div class="row">
                                ...
                                <div class="col-lg-3" id="cluster_info">
                                    <dl class="dl-horizontal" >
										...
										<dt>Referred By:</dt> <dd> 	<?php echo $getSource[0]['source_name'];  ?></dd>
								   </dl>
                                </div>
                            </div>
            ...
            <div class="col-sm-4">
                <?php $getRefDoc = mysqlSelect("a.patient_id as patient_id,a.status2 as status2,b.doc_photo as doc_photo,b.ref_id as ref_id,b.ref_name as ref_name,b.doc_spec as doc_spec,a.timestamp as timestamp","patient_referal as a inner join referal as b on a.ref_id=b.ref_id inner join doctor_hosp as d on d.doc_id=a.ref_id inner join hosp_tab as e on e.hosp_id=d.hosp_id","a.patient_id='".$GetPatient[0]['patient_id']."' and e.company_id='".$admin_id."'","a.timestamp desc","","","");
                                ...
                                <div id="contact-1">
                                    <div class="row m-b-lg">
                                        <div class="col-lg-4 text-center">
                                            <div class="m-b-sm">
                                                <img alt="image" class="img-circle" src="<?php if(!empty($getRefDocList['doc_photo'])){ echo "../Doc/".$getRefDocList['ref_id']."/".$getRefDocList['doc_photo']; } else { echo "../assets/img/anonymous-profile.png"; } ?>"
                                                     style="width: 62px">
                                            </div>
                                        </div>
                                        <div class="col-lg-8">
										<div class="ibox-tools">
                            <a href='#' data-toggle="modal" data-target="#myModalReassign">
                                 <span class='label label-danger'><i class="fa fa-reply"></i> Reassign</span>
                            </a>
                            
                        </div>
								<div class="modal inmodal" id="myModalReassign" tabindex="-1" role="dialog" aria-hidden="true">
                                <div class="modal-dialog">
                                <div class="modal-content animated bounceInRight">
                                        <div class="modal-header">
                                            <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
                                            <img src="<?php if(!empty($getRefDocList['doc_photo'])){ echo "../Doc/".$getRefDocList['ref_id']."/".$getRefDocList['doc_photo']; } else { echo "../assets/img/anonymous-profile.png"; } ?>" width="100" class="img-thumbnail" />
                                            <h4 class="modal-title"><?php echo $getRefDocList['ref_name']; ?></h4>
                                            <small class="font-bold"><?php echo $getSpec[0]['spec_name']; ?></small>
                                        </div>
											...
											<input type="hidden" name="sourceid" value="<?php echo $GetPatient[0]['patient_src']; ?>" />
											<input type="hidden" name="oldrefid" value="<?php echo $getRefDocList['ref_id']; ?>" />
                                        ...
                                        <div class="modal-body">
                                            <div class="form-group">
												<label class="control-label col-md-3 col-sm-3 col-xs-12">Select Hospital Doctor <span class="required">*</span></label>
												<div class="col-md-9 col-sm-9 col-xs-12">
												 <select class="left form-control autotab" name="selectref" id="selectref" placeholder="Select Panel Doctor" style="float:left; width:200px; padding:5px;">
																	...
																	<?php foreach($get_pro as $listDoc) {
																			$get_Ref = mysqlSelect('*','referal',"ref_id='".$listDoc['RefId']."'","","","","");
																			$get_spec = mysqlSelect('*','specialization',"spec_id='".$get_Ref[0]['doc_spec']."'","","","","");
																			$getHosp = mysqlSelect("*","hosp_tab as a left join doctor_hosp as b on a.hosp_id=b.hosp_id","doc_id='".$get_Ref[0]['ref_id']."'" ,"","","",""); 
																			//$ResponseCount = mysqlSelect("*","patient_referal","ref_id='".$listDoc['ref_id']."'" ,"","","","");
																			
																			?>	
																			
																			
																	  <option value="<?php echo $get_Ref[0]['ref_id']; ?>"  ><?php echo addslashes($get_Ref[0]['ref_name'])."&nbsp;".addslashes($get_spec[0]['spec_name']); ?></option>
																	<?php } ?>
													</select>
												</div>
											  </div>
                                        ...
                                        <div class="modal-footer">
                                            <button type="button" class="btn btn-white" data-dismiss="modal">CLOSE</button>
                                            <button type="submit" name="cmdreassign" class="btn btn-primary">REASSIGN</button>
                                        </div>
										</form>
												</div>
											</div>
                                            ...
                                            <p>
                                                <?php echo $getSpec[0]['spec_name'];  ?>
                                            </p>
                                           <?php echo $patient_status; ?><br>
									...
									<div class="col-lg-8 text-center pull-right">
										...
										<a class="btn btn-primary" href='#' onclick="return sendAptLink(<?php echo $getRefDocList['patient_id'];?>,<?php echo $getRefDocList['ref_id'];?>)"><i class="fa fa-calendar"></i> Send Appointment Link</a>
										</form>
										</div>
									</div>
									...
									<?php foreach($getchatHistory as $chatList){
											...
											{	
												...
												if(!empty($getPartner[0]['doc_photo'])){
								...
								<input type="hidden" name="ency_patient_id" value="<?php echo md5($GetPatient[0]['patient_id']); ?>" />
