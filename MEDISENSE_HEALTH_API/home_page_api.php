...
if(HEALTH_API_KEY == $data ->api_key && $data->filter_type==0 && isset($data ->userid))
	
	{
		...
		$getHospitalMap =mysqlSelect("a.doc_hosp_id as doc_hosp_id, a.doc_id as doc_id, a.hosp_id as hosp_id,md5(a.hosp_id) as hosp_encyid, b.hosp_name as hosp_name, b.hosp_addrs as hosp_addrs, b.hosp_city as hosp_city, b.hosp_state as hosp_state, b.hosp_country as hosp_country, b.geo_latitude as geo_latitude, b.geo_longitude as geo_longitude","doctor_hosp as a inner join hosp_tab as b on b.hosp_id = a.hosp_id","b.geo_longitude!=0 and b.geo_latitude!=0","","","","");
		
		
		}
		else 
		{
		$result_doctor = mysqlSelect("c.ref_id ,md5(c.ref_id) as doc_encyid, c.ref_name, c.ref_exp, c.doc_photo, c.doc_city, c.doc_qual, c.doc_interest, c.geo_latitude, c.geo_longitude","doctor_hosp as a inner join hosp_tab as b on b.hosp_id = a.hosp_id  inner join referal as c on a.doc_id=c.ref_id inner join newcitylist as d on d.city_id=b.hosp_new_city","c.doc_spec!=555 and c.anonymous_status=0 and (c.ref_address LIKE '%".$getUserLocation[0]['sub_city']."%' or d.city_name LIKE '%".$getUserLocation[0]['sub_city']."%')","c.doc_type_val asc","","","0,100");
			$doc_details= array();
			foreach($result_doctor as $result_doctorList) {
				$getDocList['ref_id']=$result_doctorList['ref_id'];
				$getDocList['doc_encyid']=$result_doctorList['doc_encyid'];
	...
	{
		$postResult = mysqlSelect("*","health_home_posts","md5(post_id)='".$data->post_id."'","","","","");
		...
		$likeUser = mysqlSelect("COUNT(like_id) as likeCount","health_home_post_like","category_id='".$postResult[0]['post_id']."' and likes='".$data->admin_id."'","","","","");
