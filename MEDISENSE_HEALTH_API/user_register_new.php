...
if(HEALTH_API_KEY == $data ->api_key && isset($data ->contact_num) && isset($data ->user_email))
{
	...
	if(empty($check_referring)){
		...
		$arrFields_user[] = 'sub_country_code';
		$arrValues_user[] = $country_code;
		...
		$success_register = array('status' => '0','message' => "Registered Successfully.", 'mobile_num' => $mobile_num,'user_name' =>$user_name,'otp_num' => $otp,'member_id' =>$member_id, 'user_email' =>$user_email,'user_id' =>$user_id);
