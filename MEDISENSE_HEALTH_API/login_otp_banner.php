...
if(HEALTH_API_KEY == $data ->api_key && isset($data ->memberid) && isset($data ->userotp))
{
	...
	if($result_referrring==true){
		...
		if(COUNT($user_consultation) > 0) {
			$success_register = array('result_otp' => "success","user_name" => $result_referrring[0]['sub_name'],"user_id" => $result_referrring[0]['login_id'],"user_mobile" => $result_referrring[0]['sub_contact'],'user_email' => $result_referrring[0]['sub_email'],"user_consult" => '1');
			echo json_encode($success_register);
		}
		else {
			$success_register = array('result_otp' => "success","user_name" => $result_referrring[0]['sub_name'],"user_id" => $result_referrring[0]['login_id'],"user_mobile" => $result_referrring[0]['sub_contact'],'user_email' => $result_referrring[0]['sub_email'],"user_consult" => '0');
			echo json_encode($success_register);
		}
