...
if(HEALTH_API_KEY == $data ->api_key && !empty($data ->patientName) && !empty($data ->mobileNum) && !empty($data ->emailID))
{ 
	...
	$TransId = $data ->transid;
		...
		$arrValues[] = '3';					// For Medisense Health
		$arrFields[] = 'system_date';
		$arrValues[] = $cur_Date;
		$arrFields[] = 'transaction_id';
		$arrValues[] = $TransId;
		$arrFields[] = 'opinion_for';
		$arrValues[] = '1';
		...
		$patientcraete=$objQuery->mysqlInsert('patient_tab',$arrFields,$arrValues);
			...
			if($txtCountry == "India"){ 
				$opcost=$getConfigInfo[0]['corona_test_cost'];
				$gst=18; //18% GST
