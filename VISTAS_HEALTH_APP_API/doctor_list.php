	...
	if($finalHash == $hashKey) 
	{
		...
		if($filter_type == 1)
		{
			...
			foreach($getDefault_doctor as $result_defaultdoctorList) 
			{
				...
				//$getDocList['active_status']	 			 =	$result_defaultdoctorList['active_status'];
				$getDocList['last_active_timestamp']	 	 =	$result_defaultdoctorList['last_active_timestamp'];
				...
				$last_active_time =	$result_defaultdoctorList['last_active_timestamp'];
				$minutes_to_add   = 3;
				...
				$arry_Field     =   array();
				$arry_Value     =   array();
				if($result_defaultdoctorList['active_status']!= "2")
				{
					if($dateTimeUTC <= $stamp)
					{
						$arry_Field[]   =   "active_status";
						$arry_Value[]   =   '1';
						$getDocList['active_status']	=	'1';
					}
					else
					{
						$arry_Field[]   =   "active_status";
						$arry_Value[]   =   '0';
						$getDocList['active_status'] =	'0';
					}
					$update_active_status = mysqlUpdate('referal',$arry_Field,$arry_Value,"ref_id='".$result_defaultdoctorList['ref_id']."'");
			...
			foreach($result_doctor as $result_doctorList)
			{
				...
				$last_active_time =	$result_doctorList['last_active_timestamp'];
				$minutes_to_add   = 3;
				$time = new DateTime($last_active_time);
				$time->add(new DateInterval('PT' . $minutes_to_add . 'M'));
				$stamp = $time->format('Y-m-d H:i:s');
				...
				$arry_Field     =   array();
				$arry_Value     =   array();
				if($result_doctorList['active_status']!= "2")
				{
					if($dateTimeUTC <= $stamp)
					{
						$arry_Field[]   =   "active_status";
					...
					$update_active_status = mysqlUpdate('referal',$arry_Field,$arry_Value,"ref_id='".$result_doctorList['ref_id']."'");
		...
		if($filter_type == 2)		
		{
			...
			$getDefault_doctor = mysqlSelect("DISTINCT(a.ref_id) as ref_id,a.last_active_timestamp as  last_active_timestamp,
			a.active_status as active_status,a.verified_by_medisense as verified_by_medisense,a.verified_by_medisense_user as verified_by_medisense_user,a.Publications_file as Publications_file,a.doc_pub as doc_pub,a.Research_Details_file as research_details_file,a.doc_research as research_details,a.Professional_Construction_file as professional_contribution_file,a.doc_contribute as professional_contribution,a.doc_interest as description, md5(a.ref_id) as doc_encyid, a.ref_name as ref_name, a.ref_exp as ref_exp, a.doc_photo as doc_photo, a.doc_city as doc_city, a.doc_country as doc_country, a.doc_qual as doc_qual, a.doc_interest as doc_interest, a.geo_latitude as geo_latitude, a.geo_longitude as geo_longitude, a.cons_charge as cons_charge, a.cons_charge_currency_type as cons_charge_currency_type","referal as a inner join doctor_langauges as b on b.doc_id=a.ref_id","a.doc_spec!=555 and a.anonymous_status=0  AND a.nova_default_doctor=1","a.ref_id DESC","","","");
			$default_doc_details= array();
			foreach($getDefault_doctor as $result_defaultdoctorList)
			{
