...
{	
	...
	if($finalHash == $hashKey)
	{
			...
			$city_latitude  = $_POST['gps_latitude'];
			$city_longitude = $_POST['gps_longitude'];
			...
			$minLatitude = $city_latitude - 5.00;
			$maxLatitude = $city_latitude + 5.00;
			
			$minLongitude = $city_longitude - 5.00;
			$maxLongitude = $city_longitude + 5.00;
			$getCity = mysqlSelect('*','newcitylist',"","","","","");
			
			
			$spec_details= array(); 
			$hosp_details= array();
			$hosp_main_details= array();
			$doctor_details= array();
			foreach($getCity as $getCityList) 
			{
				$latitudeVal =  str_replace("", "N", $getCityList['latitude']); 
				$longitudeVal = str_replace("", "E", $getCityList['longitude']); 
				$distance = Haversine($city_latitude, $city_longitude, $latitudeVal, $longitudeVal);
				if($distance < 60)
				{
					  
					$spec_details= array();
					
					$result_doctor = mysqlSelect("DISTINCT(ref_id) ,md5(ref_id) as doc_encyid,last_active_timestamp as last_active_timestamp,active_status as active_status,verified_by_medisense,verified_by_medisense_user, ref_name, ref_exp, doc_photo, doc_country_id, doc_city, doc_qual, doc_interest, geo_latitude, geo_longitude,cons_charge, Publications_file, doc_pub,Research_Details_file, doc_research,Professional_Construction_file,doc_contribute,doc_interest,cons_charge_currency_type","referal","anonymous_status=0 and doc_new_city = '".$getCityList['city_id']."'","active_status desc","","","$this1, $page_limit");

					
					foreach($result_doctor as $result_local_doctorList)
					{
				...
				$getLocalDocList['workhistory']			 =	$workhistory_details;
						...
						array_push($doctor_details, $getLocalDocList);
			 ...
			 if(COUNT($doctor_details)==$page_limit)
			{
			$page_val=$pageVal+1;
			}
			else{
			...
			$success = array('status' => "true", "doctor_list" => $doctor_details, "pagination_val" => $page_val);
