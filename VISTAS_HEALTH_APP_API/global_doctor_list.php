...
{	
	...
	if($finalHash == $hashKey)
	{
		...
		$result_global_doctor = mysqlSelect('DISTINCT(ref_id) ,md5(ref_id) as doc_encyid,last_active_timestamp,active_status as active_status,ref_name, ref_exp, doc_photo, doc_country_id, doc_city, doc_qual,  geo_latitude, geo_longitude,cons_charge, Publications_file, doc_pub,Research_Details_file, doc_research,Professional_Construction_file,doc_contribute,doc_interest,cons_charge_currency_type','referal',"(doc_country_id)!='".$country_id."' AND anonymous_status=0 and doc_type='featured' ","active_status desc","","","","$this1, $page_limit");
		
		
		
		

			$globalDoc_details= array();
			foreach($result_global_doctor as $result_local_doctorList) 
			{
				...
				array_push($globalDoc_details, $getLocalDocList);
			
			}
			if(COUNT($result_global_doctor)==$page_limit)
			{
				$page_val=$pageVal+1;
			}
			else
			...
			$success = array('status' => "true", "global_doctor_array" => $globalDoc_details, "pagination_val" => $page_val);
