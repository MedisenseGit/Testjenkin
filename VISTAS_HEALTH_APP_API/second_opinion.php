...
if(!empty($user_id) && !empty($finalHash)) {	
	...
	$txtName 	= $_POST['member_name'];
		...
		$getSrcInfo = mysqlSelect("*","source_list","partner_id='".$login_id."' and src_type='1'" ,"","","","");
		$src_id = $getSrcInfo[0]['source_id'];
			...
			foreach($_FILES['file-3']['tmp_name'] as $key => $tmp_name )
			{
					...
					if(!empty($file_name))
					{
						...
						$pat_attach=mysqlInsert('patient_attachment',$arrFields_attach,$arrValues_attach);
						$attachid= $pat_attach;
			...
			$createreferral=mysqlInsert('patient_referal',$arrFields1,$arrValues1);
			...
			$getPatInfo = mysqlSelect("*","patient_tab","patient_id='".$id."'" ,"","","","");
			...
			if($getPatInfo[0]['patient_mob']!="")
			{
				...
				$msg = "Hi ".$getPatInfo[0]['patient_name'].", We have registered your medical query with number: ". $getPatInfo[0]['patient_id'] . ", Kindly check your email for further details. Thanks.\n MedisenseHealth.com";
				send_msg($mobile,$msg);
		...
		$success_opinion = array('result' => "success", 'status' => '1', 'message' => "Your second opinion request has been sent successfully. \n\nYou will receive an update within 24-48hrs. ", 'err_msg' => '');
