<?php
//DEFINE API KEY
define('API_KEY', 'AIzaSyAxlWvMHPxDZwdV5mzYp7ipWqjKmdq8TBI');
...
function hmacHashFunction($key, $value) {
	return hash_hmac('sha256', utf8_encode($key), utf8_encode($value), false);
}

function checkAccessTokenFunction($objQuery, $timestamp, $method, $body, $user_id, $device_id) {
	$result_access = mysqlSelect('id, accessToken','login_sessions',"login_id='".$user_id."' AND device_id='".$device_id."'","id DESC","","","1");
	 ...
	 if($method == "GET") {
		...
		if(!empty($result_access[0]['accessToken'])) {
			  $hash1 = hmacHashFunction($timestamp, $result_access[0]['accessToken']);
			  $finalHash = hmacHashFunction($hash1, json_encode($body)); 	// Body is json encoded bcoz Its a POST Request
		}
	}
	else {
		$finalHash = "";
