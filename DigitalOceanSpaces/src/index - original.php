...
try {
    ...
    $stream = fopen('bird-thumbnail.jpg', 'r+');

    //If file uploaded
    //$filesystem->writeStream("{$upload_dir}/".$_FILES[$uploadName]['name'], $stream, ['visibility' => $file_visibility]);

    $filesystem->writeStream("{$upload_dir}/bird-thumbnail.jpg", $stream, ['visibility' => $file_visibility]);
    ...
    $file_url = "https://{$space_name}.{$endpoint}/{$upload_dir}/bird-thumbnail.jpg";
