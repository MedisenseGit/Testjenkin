$(document).ready(function(){
  ...
  var afterFilter = function(result, jQ){
    ...
    var checkboxes1  = $("#specialize_criteria :input:gt(0)");
    ...
    checkboxes.each(function(){
	 ...
	 checkboxes1.each(function(){
      ...
      if(result.length > 0){
        count = jQ.where({ 'outline': c.val() }).count;
      }
      c.next().text(c.val() + '(' + count + ')')
    });
  ...
  FJS.addCriteria({field: 'outline', ele: '#specialize_criteria input:checkbox'});

  /*
   * Add multiple criterial.
    FJS.addCriteria([
      {field: 'genre', ele: '#genre_criteria input:checkbox'},
      {field: 'outline', ele: '#specialize_criteria', input:checkbox'}
    ])
  */
