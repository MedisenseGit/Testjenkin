	...
	if($finalHash == $hashKey) {
		...
		if(!empty($_POST['investigation_id']))	{	 
			while (list($key, $val) = each($_POST['investigation_id'])) {
				...
				$update_invest=mysqlUpdate('patient_temp_investigation',$arrFiedInvest,$arrValueInvest, "pti_id = '".$investigation_id."'");
				...
				{
					$update_trend=mysqlUpdate('trend_analysis',$arrFieldTrend,$arrValueTrend,"date_added='".$cur_Date."' and patient_id = '".$patient_id."' and patient_type='1'");
				}
				else
				{
