...
if(!empty($doctor_id) && !empty($finalHash)) {
	...
	if($finalHash == $hashKey) {
		...
		if($listing_type=="Blog"){
			$checkLike = mysqlSelect("COUNT(like_id) as checkUser","home_post_like","likes='".$user_id."' and category_id='".$event_id."' and category_type='".$listing_type."' and user_type=2","","","","");
			$numlikes = mysqlSelect("COUNT(like_id) as countLike","home_post_like","category_id='".$event_id."' and category_type='".$listing_type."'","","","","");
		
			$stuff= array();
			$commentResult = mysqlSelect("*","home_post_comments","topic_id='".$event_id."' and topic_type='".$listing_type."'","","","","");
			$commentCount = mysqlSelect("COUNT(comment_id) as CountComment","home_post_comments","topic_id='".$event_id."' and topic_type='".$listing_type."'","","","","");
			foreach($commentResult as $postResultList) {
				...
				if($postResultList['login_User_Type'] == 1) {  //For Standard Doctor
					$result = mysqlSelect("partner_name,doc_photo","our_partners","partner_id='".$postResultList['login_id']."'","","","","");
					 $stuff["username"] = $result[0]['partner_name'];
						//Profile Pic
						if(!empty($result[0]['doc_photo'])){
						$userimg="https://medisensemd.com/Refer/partnerProfilePic/".$postResultList['login_id']."/".$result[0]['doc_photo']; 
						}else{
						$userimg="https://medisensemd.com/assets/img/anonymous-profile.png";
						}		
			 		$stuff["user_image"] = $userimg;
				}
				else if($postResultList['login_User_Type'] == 2) {  //For Premium Doctors
					$result = mysqlSelect("ref_name,doc_photo","referal","ref_id='".$postResultList['login_id']."'","","","","");
					 $stuff["username"] = $result[0]['ref_name'];	

					//Profile Pic
					if(!empty($result[0]['doc_photo'])){ 
					$userimg="https://medisensemd.com/Doc/".$postResultList['login_id']."/".$result[0]['doc_photo']; 
					}else{
					$userimg="https://medisensemd.com/assets/img/anonymous-profile.png";
					}	
				 	 $stuff["user_image"] = $userimg;
					...
					$result = mysqlSelect("company_name,company_logo","compny_tab","company_id='".$postResultList['login_id']."'","","","","");
					 $stuff["username"] = $result[0]['company_name'];	

					//Profile Pic
					if(!empty($result[0]['company_logo'])){ 
					$userimg="https://medisensemd.com/Hospital/company_logo/".$postResultList['login_id']."/".$result[0]['company_logo']; 
					}else{
					$userimg="https://medisensemd.com/assets/img/anonymous-profile.png";
					}	
				 	 $stuff["user_image"] = $userimg;
