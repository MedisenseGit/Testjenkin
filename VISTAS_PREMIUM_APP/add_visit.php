...
if(!empty($doctor_id) && !empty($finalHash)) {
	...
	if($finalHash == $hashKey) {  */
		...
		$patient_education =  $data['patient_education'];
			...
			if(!empty($data['followup_dates'])) {
				$arrFieldsPE[] = 'next_followup_date';
				$arrValuesPE[] = date('Y-m-d',strtotime($data['followup_dates']));
			}
			...
			if(!empty($data['visit_entry_date'])) {
				...
				$arrValuesPE[] = date('Y-m-d H:i:s',strtotime($data['visit_entry_date']));
			}
			else {
				$arrFieldsPE[] = 'date_time';
		...
		if ($chkInvestSaveTemplate == 1)
		{
			...
			$invets_template_id = $insert_patient_episode_prescription_template; //Get invets_template_id
					
			$getChosenInvset= mysqlSelect("*","patient_temp_investigation","doc_id='".$admin_id."' and doc_type='1' and episode_id='".$episode_id."' and patient_id='".$patient_id."'","","","","");
		...
		if($chkExamSaveTemplate == 1) {
				...
				$getChosenExam= mysqlSelect("*","doc_patient_examination_active","doc_id='".$admin_id."' and doc_type='1' and episode_id='".$episode_id."' and patient_id='".$patient_id."'","","","","");
		...
		if(!empty($data['prescription_ppID']))  {
			while (list($key, $val) = each($data['prescription_ppID'])) {
				...
				if($presc_pp_id == 0) {
					...
					/*$chkProduct= mysqlSelect("pp_id,freq_count","doctor_frequent_medicine","pp_id='".$get_ppid."'","","","","");
					$arrFileds_freq = array();
					$arrValues_freq = array();
					if($chkProduct == true)
					{
			...
			$arrValueTrend[]="1";
			$checkTrend= mysqlSelect("*","trend_analysis","date_added='".$cur_Date."' and patient_id='".$patient_id."' and patient_type='1'","","","","");
    ...
    /*      $chkPatientAppTab = mysqlSelect("*","appointment_token_system","patient_id='".$patient_id."' and doc_id='".$admin_id."' and doc_type='1' and hosp_id='".$_SESSION['login_hosp_id']."' and app_date='".date('Y-m-d')."'","","","","");
          ...
          $chkPatientAppTab = mysqlSelect("*","appointment_token_system","patient_id='".$patient_id."' and doc_id='".$admin_id."' and doc_type='1' and hosp_id='".$hospital_id."' and app_date='".date('Y-m-d')."'","","","","");
					if(count($chkPatientAppTab)>0)
					{
						//Update Appointment Status as  -'Consulted'
						$arrFieldsAppStatus[] = 'status';
						...
						$update_appoint_trans=mysqlUpdate('appointment_transaction_detail',$arrFieldsAppTransStatus,$arrValuesAppTransStatus,"Visiting_date = '".date('Y-m-d')."' and patient_id='".$patient_id."'and pref_doc='".$admin_id."' and hosp_id='".$hospital_id."'");
