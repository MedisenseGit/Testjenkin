...
if(!empty($doctor_id) && !empty($finalHash)) 
{
	if($finalHash == $hashKey) 
	{
		...
		if($patient_note != '' || $refer_to != '' || $specialization != '')
		{
			...
			$select_doc_my_pat = mysqlSelect("patient_name, patient_age, DOB, patient_email, patient_gen, merital_status, qualification, height_cm, height, weight, hyper_cond, smoking, alcoholic, diabetes_cond, pat_blood, drug_abuse, other_details, family_history, prev_inter, neuro_issue, kidney_issue, contact_person, profession, patient_mob, patient_loc, pat_state, pat_country, patient_addrs, tele_communication, patientEMR_consent, TImestamp, user_id, doc_id, patient_image, system_date, transaction_id, data_source, member_id, pat_bp, pat_thyroid, pat_cholestrole, pat_epilepsy, pat_asthama, doc_video_link, pat_video_link, teleconsult_status, subscriber_id","doc_my_patient","patient_id = '". $patient_id ."' ","","","","");
			...
			$insert_doc_my_pat = mysqlInsert('doc_my_patient',$arrFieldsDMP,$arrValuesDMP);
