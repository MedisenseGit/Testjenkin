...
if(!empty($doctor_id) && !empty($finalHash)) {
	...
	if($finalHash == $hashKey) {
		...
		$updateProvider=mysqlUpdate('referal',$arrFields,$arrValues,"ref_id='".$admin_id."'");
		$id=$admin_id;	
		/* Uploading image file */ 
			...
			if(!empty($_POST['doc_specialization'])) {
				mysqlDelete('doc_specialization',"doc_id='".$admin_id."' and doc_type='1'");
				...
				while (list($key, $val) = each($_POST['doc_specialization'])) {
					...
					$insert_spec=mysqlInsert('doc_specialization',$arrFileds_spec,$arrValues_spec);
			...
			if(!empty($_POST['doc_hospital'])) {
				mysqlDelete('doctor_hosp',"doc_id='".$admin_id."'");
				...
				while (list($key, $val) = each($_POST['doc_hospital'])) {
					...
					$insert_hosp=mysqlInsert('doctor_hosp',$arrFileds_hosp,$arrValues_hosp);
