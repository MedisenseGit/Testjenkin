...
if(!empty($doctor_id) && !empty($finalHash)) {
	...
	if($finalHash == $hashKey) {
		...
		$userupdate=mysqlUpdate('doc_my_patient',$arrFields,$arrValues, "patient_id = '".$patient_id."' ");
	
		/* Add Drug Allergy */	
		if(!empty($_POST['allergy_id'])) {
			mysqlDelete('doc_patient_drug_allergy_active',"doc_id='".$admin_id."' and doc_type ='1' and patient_id='".$patient_id."'");
			...
			while (list($key, $val) = each($_POST['allergy_id'])) {
				...
				$check_allergy = mysqlSelect("*","doc_patient_drug_allergy_active","allergy_id='".$allergy_ID."' and doc_id='".$admin_id."' and doc_type='1' and patient_id='".$patient_id."'","","","","");
				if(count($check_allergy)>0){
					mysqlDelete('doc_patient_drug_allergy_active',"allergy_id='".$allergy_ID."'");
				}
				else {
					$arrFileds = array();
					$arrValues = array();
		...
		if(!empty($_POST['abuse_id']))	{
			mysqlDelete('doc_patient_drug_active',"doc_id='".$admin_id."' and doc_type ='1' and patient_id='".$patient_id."'");
			...
			while (list($key, $val) = each($_POST['abuse_id'])) {
				...
				if($abuse_ID == 0) {
					...
					$insert_drugs=mysqlInsert('drug_abuse_auto',$arrFileds_drug,$arrValues_drug);
				...
				$check_drug = mysqlSelect("*","doctor_frequent_drug_abuse","drug_abuse_id='".$drug_id."' and doc_id='".$admin_id."' and doc_type='1'","","","","");
					...
					if(count($check_drug)>0){
						...
						$update_icd=mysqlUpdate('doctor_frequent_drug_abuse',$arrFieldsDrugAbuseFreq,$arrValuesDrugAbuseFreq,"fda_id = '".$check_drug[0]['fda_id']."'");
					}
					else{
						...
						$arrFieldsDrugAbuseFreq[] = 'freq_count';
						$arrValuesDrugAbuseFreq[] = "1";
						$insert_freq_symp=mysqlInsert('doctor_frequent_drug_abuse',$arrFieldsDrugAbuseFreq,$arrValuesDrugAbuseFreq);
		...
		if(!empty($_POST['family_id'])) {
				mysqlDelete('doc_patient_family_history_active',"doc_id='".$admin_id."' and doc_type ='1' and patient_id='".$patient_id."'");	
				...
				while (list($key, $val) = each($_POST['family_id'])) {
					...
					$insert_family=mysqlInsert('family_history_auto',$arrFileds_family,$arrValues_family);
					$family_id = $insert_family; //Get Family Id
				}
				else {
					$family_id = $family_ID;
				}
				...
				$insert_familyhistory =mysqlInsert('doc_patient_family_history_active',$arrFileds,$arrValues);
				
				$check_family_history = mysqlSelect("*","doctor_frequent_family_history","family_history_id='".$family_id."' and doc_id='".$admin_id."' and doc_type='1'","","","","");
					...
					if(count($check_family_history)>0){
						...
						$update_family=mysqlUpdate('doctor_frequent_family_history',$arrFieldsFamilyFreq,$arrValuesFamilyFreq,"ffh_id = '".$check_family_history[0]['ffh_id']."'");
					}
					else{
						...
						$arrFieldsFamilyFreq[] = 'freq_count';
						$arrValuesFamilyFreq[] = "1";
						$insert_freq_family=mysqlInsert('doctor_frequent_family_history',$arrFieldsFamilyFreq,$arrValuesFamilyFreq);
		...
		$getFrequentDrugAllery = mysqlSelect("*","doc_patient_drug_allergy_active","doc_id='".$admin_id."' and doc_type='1' and patient_id='".$patient_id."'","allergy_id DESC","","","");
		...
		$selectDrugAbuse= mysqlSelect("*","drug_abuse_auto"," (doc_id='0' and doc_type='0') or (doc_id='".$admin_id."' and doc_type='1')","drug_abuse asc","","","");
		...
		$selectFamilyHistory= mysqlSelect("*","family_history_auto"," (doc_id='0' and doc_type='0') or (doc_id='".$admin_id."' and doc_type='1')","family_history asc","","","");
