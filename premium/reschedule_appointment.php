...
if($_SESSION['appointClick']==1 && !isset($_POST['appointmentUpcoming']) && !isset($_POST['appointmentAll']) && !isset($_POST['appointmentToday']))
{	
	...
	$appUpcomingResult = mysqlSelect("b.patient_id as patient_id,b.patient_name as patient_name,a.transaction_id as appoint_trans_id , a.visiting_date as app_date,a.time_slot as app_time,a.pay_status as status","patients_transactions as a inner join  patients_appointment as  b  on a.patient_id=b.patient_id ","a.doc_id='".$admin_id."'  and a.hosp_id='".$_SESSION['login_hosp_id']."' and a.visiting_date>='".$curdate."'","a.visiting_date ASC","","","");

	$_SESSION['appointClick']=1;
}
else if($_SESSION['appointClick']==2 && !isset($_POST['appointmentAll']) && !isset($_POST['appointmentUpcoming']) && !isset($_POST['appointmentToday']))
{
	...
	$appointmentResult = mysqlSelect("b.patient_id as patient_id,b.patient_name as patient_name,a.transaction_id as appoint_trans_id , a.visiting_date as app_date,a.time_slot as Timing,a.pay_status as pay_status","patients_transactions as a inner join  patients_appointment as  b  on a.patient_id=b.patient_id","doc_id='".$admin_id."' and hosp_id='".$_SESSION['login_hosp_id']."'","Visiting_date desc","","","");
	
	$_SESSION['appointClick']=2;
}	
else if(isset($_POST['appointmentUpcoming']))
{	
	//$appUpcomingResult = mysqlSelect("*","appointment_token_system","doc_id='".$admin_id."' and doc_type='1' and hosp_id='".$_SESSION['login_hosp_id']."' and app_date>='".$curdate."'","app_date ASC","","","");
...
else
{
	...
	$appointmentToday = mysqlSelect("b.patient_id as patient_id,b.patient_name as patient_name,a.transaction_id as appoint_trans_id , a.visiting_date as app_date,a.time_slot as app_time,a.pay_status as status","patients_transactions as a inner join  patients_appointment as  b  on a.patient_id=b.patient_id","doc_id='".$admin_id."' and hosp_id='".$_SESSION['login_hosp_id']."' and a.pay_status='Cancelled' ","Visiting_date desc","","","");
...
<head>
	...
	function getDateFun(date_result,day,selec_date)
	{
		...
		var docId = <?php echo $admin_id; ?>;
		var hosp_id=<?php echo $_SESSION["login_hosp_id"]; ?>;//$('#se_depart :selected').val();
					 ...
					 <table class="footable table table-stripped" data-page-size="100" data-filter=#filter>
						...
						<tbody>
							...
							<tr>
								...
								<td><center>
									<?php if($Todaylist['status']!="Cancelled"){ ?>	<a href="Reschedule?a=<?php echo $Todaylist['appoint_trans_id']; ?>" ><i class="fa fa-edit"></i></a> </center><?php } ?>
								...
								<form method="post" enctype="multipart/form-data" class="form-horizontal form-label-left"  action="add_details.php"  name="frmAddPatient" id="frmAddPatient">
									...
									<div class="form-group">
									<label class="col-sm-2 control-label" for="date_added">Preferred Date 1<span class="required">*</span></label>
								...
								if($Todaylist['set_diation_timer']=="00:00:00" && $Todaylist['dilation_status']==1)
								{
						...
						<tbody>
							...
							function getDocTiming<?php echo $list['appoint_trans_id']; ?>(val) {
								...
								<td><center>
									<?php if($list['status']!="Cancelled"){ ?>	<a href="Reschedule?a=<?php echo $list['appoint_trans_id']; ?>" ><i class="fa fa-edit"></i></a> </center><?php } ?>
								...
								<form method="post" enctype="multipart/form-data" class="form-horizontal form-label-left"  action="add_details.php"  name="frmAddPatient" id="frmAddPatient">
									...
									<!--<div class="form-group">
									<div class="form-group">
									<label class="col-sm-2 control-label" for="date_added">Preferred Date 2<span class="required">*</span></label>
						...
						<tr>
							...
							<tr>
								...
								<td><center>
									<?php if($Alllist['pay_status']!="Cancelled"){ ?>	<a href="Reschedule?a=<?php echo $Alllist['appoint_trans_id']; ?>" ><i class="fa fa-edit"></i></a> </center><?php } ?> <!--data-toggle="modal" data-target="#myModal<?php echo $Alllist['appoint_trans_id']; ?>"-->
                            ...
                            <div class="modal-body">
                                <div class="form-group">
									<div class="form-group">
										<label class="col-sm-2 control-label" for="date_added">Preferred Date 3<span class="required">*</span></label>
										<div class="col-sm-4">
			...
			<div class="tabs-container">
                        ...
                        <div class="tab-content">
                            <div id="tab-1" class="tab-pane active">
								...
								//$getBookingDetail	= mysqlSelect("*","appointment_transaction_detail","appoint_trans_id='".$_GET['a']."'","","","","");
								
								$getBookingDetail	= mysqlSelect("a.patient_id as patient_id,a.service_type as appointment_type,a.reference_id as reference_id,a.referring_hosp as referring_hosp,a.referring_doc as referring_doc,a.patientEMR_consent as patientEMR_consent,a.transaction_id as appoint_trans_id"," patients_transactions as a","a.transaction_id='".$_GET['a']."'","","","","");
								
								//$get_PatientDetails = mysqlSelect("patient_id,patient_name,patient_age,patient_email,patient_mob,patient_gen,patient_loc,pat_state,pat_country,patient_addrs,DOB","doc_my_patient","patient_id='".$getBookingDetail[0]['patient_id']."'","","","","");	
								
								$get_PatientDetails = mysqlSelect("a.patient_id as patient_id,a.patient_name as patient_name,b.patient_age as patient_age,a.patient_email as patient_email,a.patient_mobile as patient_mob,a.patient_gender as patient_gen,b.city as patient_loc,b.state as pat_state,b.country as pat_country,b.address as patient_addrs,a.patient_dob as DOB","patients_appointment as a INNER JOIN patients_transactions as b on a.patient_id=b.patient_id","a.patient_id='".$getBookingDetail[0]['patient_id']."'","","","","");	
								
								
								
								
								
								$child_tab 			= mysqlSelect("*","child_tab","patient_id='".$getBookingDetail[0]['patient_id']."'","","","","");
								...
								<div class="col-sm-12">  
									<div class="radio radio-info radio-inline">
										<input type="radio"   class="iradio_square-green" value="1" required="required" name="appointment_type" <?php if($getBookingDetail[0]['appointment_type']==0){ ?>checked <?php } ?> id="todayRadio">
										<label for="inlineRadio6" class="m-t-xs"><b> Walk-In Patient </b></label>
                                    </div>
                                    <div class="radio radio-info radio-inline ">
										<input type="radio"  class="iradio_square-green" value="2" required="required" name="appointment_type" <?php if($getBookingDetail[0]['appointment_type']==1){ ?>checked <?php } ?> id="future">
										<label for="inlineRadio7" class="m-t-xs"><b> Appointments </b></label>
                        			</div>											
									<div class="radio radio-info radio-inline ">
										<input type="radio"  class="iradio_square-green" value="3" required="required" name="appointment_type" <?php if($getBookingDetail[0]['appointment_type']==2){ ?>checked <?php } ?> id="onlineConsult">
															...
															<option value="">Select Date</option>
																...
																{ ?>
																	...
																	{ 
																		...
																		<option value="<?php echo date('Y-m-d', $date);?>" >
																		<?php
																		if($i==0) { echo "Today";} else if($i==1){ echo "Tomorrow";} else 
																		{ 
							...
							<div class="form-horizontal">
								<div class="form-group" id="date_section" >
									<div class="row well well-lg">
										<label><b>Appointment Time / Date Slot</b></label><br />
												...
												{ 
													...
													{ 
														...
														{ 
															...
															//echo $selected_date;
															?>
															<!--input type='text' id='se_date2' name='se_date2' value='<?php echo date('Y-m-d', $date); ?>' class="card card-block mx-2" style="min-width: 100px;" onclick="getDateFun(this.value,'<?php  echo date('D', $date) ?>');"  readonly /-->
															<input type='button' id='check_date' name='check_date' value='<?php echo date('D, d M', $date); ?>'style="min-width: 120px;background-color:rgba(0, 0, 0, .03); border:solid 1px; border-radius:5px; margin-right:15px;padding-bottom:5px; padding-top:5px;margin-bottom:10px;" onclick="getDateFun(this.value,'<?php  echo date('D', $date) ?>','<?php echo $selected_date; ?>');"  readonly />
							...
							<form enctype="multipart/form-data" method="post" action="add_details.php"  name="frmAddPatient" autocomplete="off" >
								...
								<input type="hidden" id="hosp_id"       name="hosp_id"    	  value="<?php echo $_SESSION['login_hosp_id']; ?>" />
                                    ...
                                    <div class="col-sm-4"><input type="text" name="se_pat_name" required="required" class="form-control"  value="<?php echo $get_PatientDetails[0]['patient_name']; ?>"></div>
                                
									<?php	if($getDocSpec[0]['spec_id']=="32"){ ?>
									
									<label class="col-sm-2 control-label">DOB</label>
                                    <div class="col-sm-4">									
									<input id="dateadded" name="date_birth" type="text" value="<?php echo date('m/d/Y',strtotime($get_PatientDetails[0]['DOB']));?>" placeholder="" class="form-control" >
                                  
									</div>
								<?php }  else{ ?>			  
									<label class="col-sm-2 control-label">Age </label>

                                    <div class="col-sm-4"><input type="text" name="se_pat_age" class="form-control" value="<?php echo $get_PatientDetails[0]['patient_age'];?>"></div>
                               	<?php } ?>
							   </div>
								<div class="form-group">
								<label class="col-sm-2 control-label">Gender <span class="required">*</span></label>
                                      <div class="col-sm-6">  <div class="radio radio-info radio-inline">
                                            <input type="radio" id="inlineRadio1" value="1" required="required" name="se_gender" <?php if($get_PatientDetails[0]['patient_gen']=="1") { ?> checked <?php } ?>>
                                            <label for="inlineRadio1"> Male </label>
                                        </div>
                                        <div class="radio radio-info radio-inline">
                                            <input type="radio" id="inlineRadio2" value="2" required="required" name="se_gender" <?php if($get_PatientDetails[0]['patient_gen']=="2") { ?> checked <?php } ?>>
                                            <label for="inlineRadio2"> Female </label>
                                        </div>
										<div class="radio radio-info radio-inline">
                                            <input type="radio" id="inlineRadio2" value="3" required="required" name="se_gender" <?php if($get_PatientDetails[0]['patient_gen']=="3") { ?> checked <?php } ?>>
                                    ...
                                    <div class="col-sm-4">
										<select data-placeholder="Choose a State..." class="form-control" name="se_state" id="se_state" tabindex="2">
											<?php
													...
													?>
													<option value="<?php echo $StateList["state_name"];	?>" <?php if($get_PatientDetails[0]['pat_state']==$StateList["state_name"]){ ?> selected <?php } ?>><?php echo $StateList["state_name"]; ?></option>
                                    ...
                                    <div class="col-sm-10"><select data-placeholder="Choose a Country..." class="chosen-select" name="se_country"  tabindex="2" onchange="return getState(this.value); ">
											<option value="Qatar" selected>Qatar</option>
												...
												?> 
														...
														<option value="<?php echo stripslashes($CountryList['country_name']); ?>" <?php if($get_PatientDetails[0]['pat_country']==$CountryList["country_name"]){ ?> selected <?php } ?>/>
								...
								<div id="dispCity"></div>
									...
									?>
									<option value="<?php echo $listDoc['referred_doc_id']; ?>" <?php if($getBookingDetail[0]['reference_id'] == $listDoc['referred_doc_id']){ ?> selected <?php } ?>><?php echo $listDoc['referral_name']; ?> </option>
								...
								<div class="form-group"><label class="col-sm-2 control-label">Referring Hospital </label>
									...
									foreach($select1 as $listDoc){ 
									?>
									<option value="<?php echo $listDoc['hos_out_ref_id']; ?>" <?php if($getBookingDetail[0]['referring_hosp'] == $listDoc['hos_out_ref_id']){ ?> selected <?php } ?>><?php echo $listDoc['hospital_name']; ?> </option>
								...
								<div class="form-group"><label class="col-sm-2 control-label">Referring Doctor </label>
									...
									<?php $getRefDoctor = mysqlSelect("a.doc_out_ref_id as Ref_Id,a.doctor_name as Doc_name,b.hospital_name as Hosp_name,c.spec_name as Department","doctor_in_referral as a inner join hospital_in_referral as b on b.hos_out_ref_id=a.ref_hosp_id inner join specialization as c on c.spec_id=a.doc_specialization","b.hos_out_ref_id='".$getBookingDetail[0]['referring_hosp']."'","a.doctor_name asc","","","");
									?>
										<option value="">Select Doctor</option>
									<?php $i=30;
										foreach($getRefDoctor as $DocList) {	
											
									?>
											<option value="<?php echo stripslashes($DocList['Ref_Id']); ?>" <?php if($getBookingDetail[0]['referring_doc'] == $DocList['Ref_Id']){ ?> selected <?php } ?>>
								...
								<div class="form-group teleConsultDisp" style="padding-top:10px;<?php if($getBookingDetail[0]['appointment_type']==0){ ?>style="display:none;" <?php } ?>">	  
									<label class="col-sm-2 control-label"> </label>
									
									<div class="col-sm-10">
									<input type="checkbox" class="i-checks" name="chkTeleCom" value="1" <?php if($getBookingDetail[0]['tele_communication']==1){ ?>checked<?php } ?>> I'm ready for teleconsultation
									</div>
								</div>
								<div class="form-group" style="padding-top:10px;">
									...
									<div class="col-sm-10">
									 <input type="checkbox"  class="i-checks" name="chkPatConsent" value="1" <?php if($getBookingDetail[0]['patientEMR_consent']==1){ ?>checked<?php } ?>> Patient agree for our Institute to share the EMR with Professional Health CarePartners (Diagnostic, Pharmacy)
								...
								<input type="hidden" name="patient_id" value="<?php echo $get_PatientDetails[0]['patient_id']; ?>" />
								<input type="hidden" name="trans_id" value="<?php echo $getBookingDetail[0]['appoint_trans_id']; ?>" />
								<input type="hidden" name="trans_id" value="<?php echo $getBookingDetail[0]['appoint_trans_id']; ?>" />								
								<div class="form-group">
								<div class="col-sm-6 pull-right">
								<button type="submit" name="reschedule_appointment" class="btn btn-primary block full-width m-b ">Reschedule</button>
								</div>
								</div>
							</form>
								</div>
