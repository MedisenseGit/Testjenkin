...
if(isset($_POST['appointmentMissed']))
{	
	$appUpcomingMissed = mysqlSelect("b.patient_id as patient_id,b.patient_name as patient_name,a.transaction_id as appoint_trans_id , a.visiting_date as app_date,a.time_slot as app_time,a.pay_status as status","patients_transactions as a inner join  patients_appointment as  b  on a.patient_id=b.patient_id","doc_id='".$admin_id."' and hosp_id='".$_SESSION['login_hosp_id']."' and a.pay_status='Missed' ","Visiting_date desc","","","");
	
	$_SESSION['appointClick']=4;
}
else if(isset($_POST['appointmentComplete']))
{	
	$appUpcomingcompl = mysqlSelect("b.patient_id as patient_id,b.patient_name as patient_name,a.transaction_id as appoint_trans_id , a.visiting_date as app_date,a.time_slot as app_time,a.pay_status as status","patients_transactions as a inner join  patients_appointment as  b  on a.patient_id=b.patient_id","doc_id='".$admin_id."' and hosp_id='".$_SESSION['login_hosp_id']."' and a.pay_status='VC Confirmed' ","Visiting_date desc","","","");

	$_SESSION['appointClick']=5;
}
...
$result_patTreat 	= 	mysqlSelect('COUNT(episode_id) AS patient_treated','doc_patient_episodes',"admin_id='".$admin_id."'");

$profile_stat 		= 	$result[0]['profile_percentage'];
$patient_Treated 	= 	$result_patTreat[0]['patient_treated'];
$patientAppreciated = 	'65';
$totalPatients 		= 	mysqlSelect('COUNT(a.patient_id) AS patient_id','patients_appointment as a inner join patients_transactions as b on a.patient_id=b.patient_id',"a.doc_id='".$admin_id."'");


$totalPatients 		= 	$totalPatients[0]['patient_id'];
$responseRate 		= 	((int) $patient_Treated / (int) $totalPatients) * 100;
$responseMinutes 	= 	mysqlSelect('MIN(NULLIF(response_duration, 0)) as response_duration','appointment_accept_reject',"doc_id='".$admin_id."'");
...
function getDateFun(date_result,day,selec_date)
{
	...
	var hosp_id=<?php echo $_SESSION["login_hosp_id"]; ?>;
	$.ajax({
		type: "POST",
	...
	<div class="col-lg-3">
		<a href="#">
			<div class="widget style1 red-bg">
				...
				<div class="col-xs-3">
						<?php echo $result_patTreat[0]['patient_treated']; ?>
					</div>
					<div class="col-xs-9 text-left">
						<h3 class="font-bold">Patient Treated</h3>
					</div>
				</div>
			</div>
		</a>
	...
	<div class="col-lg-3">
		<a href="#">
			<div class="widget style1 navy-bg">
				<div class="row vertical-align">
					<div class="col-xs-3">
						<?php echo $responseMinutes[0]['response_duration'];; ?>
					</div>
					<div class="col-xs-9 text-left">
						<h3 class="font-bold">Mins Response</h3>
					</div>
				</div>
			</div>
		</a>
	...
	<div class="col-lg-3">
		<a href="#">
			<div class="widget style1 lazur-bg">
				<div class="row vertical-align">
					<div class="col-xs-3">
						<?php echo $patientAppreciated; ?>
					</div>
					<div class="col-xs-9 text-left">
						<h3 class="font-bold">Patient Appreciated</h3>
					</div>
				</div>
			</div>
		</a>
	...
	<div class="col-lg-3">
		<a href="#">
			<div class="widget style1 blue-bg">
				<div class="row vertical-align">
					<div class="col-xs-3">
						<?php echo round($responseRate); ?>
					</div>
					<div class="col-xs-9 text-left">
						<h3 class="font-bold">Response Rate</h3>
					</div>
				</div>
			</div>
		</a>
...
<div class="ibox float-e-margins">
	<div class="ibox-title">
		...
		<div class="btn-group pull-left">
			...
			<form method="post" name="changeAppType" >
				...
				<button class="btn btn-white" name="appointmentMissed"  type="submit">Missed</button>
				<button class="btn btn-white" name="appointmentComplete"  type="submit">Completed</button>
		...
		<table class="footable table table-stripped" data-page-size="100" data-filter=#filter>
			<thead>
				<tr>
					...
					<th>Status</th>
					<th>Re-Schedule</th>
					<?php if($getDocEMR[0]['spec_group_id']==2){ ?> 
		...
		?>
			...
			<tbody>
				...
				<tr>
					...
					<td><a href="<?php echo $_SESSION['EMR_URL'].md5($Todaylist['patient_id']).'&TID='.md5($Todaylist['appoint_trans_id']) . '_' .$Todaylist['appoint_trans_id'] ;?>"><?php echo $Todaylist['patient_name']; ?></a></td>
					<td style="min-width:150px;" ><?php echo date('d-m-Y',strtotime($Todaylist['app_date']))." | ".$list['app_time']; ?></td>
					<td>
					...
					<td>
						<center>
							<?php if($Todaylist['status']!="Cancelled"){ ?>	<a href="Reschedule?a=<?php echo $Todaylist['appoint_trans_id']; ?>" ><i class="fa fa-edit"></i></a> 
						</center>
						<?php } ?>
		...
		<table class="footable table table-stripped" data-page-size="100" data-filter=#filter>
			...
			<tbody>
				<tr>
					...
					<td colspan="3">Completed Appointments</td>
				</tr>
				<?php 
					
					foreach($appUpcomingcompl as $list)
					{ 
						$utc_slot = $list['app_time'];
						$UTCObj   = new DateTime($utc_slot, new DateTimeZone("UTC"));
						$LocalObj = $UTCObj;
						$LocalObj->setTimezone(new DateTimeZone($timezone));
				...
				?>
				<tr>
					<td><a href="<?php echo $_SESSION['EMR_URL'].md5($list['patient_id']).'&TID='.md5($list['appoint_trans_id']) . '_' .$list['appoint_trans_id'];?>"><?php echo $list['patient_name']; ?></a></td>
					<td style="min-width:200px;" ><?php echo date('d-m-Y',strtotime($list['app_date']))." | ".$time_slot; ?></td>
					<td>
						<div class="btn-group pull-right">
							<?php 
	...
	<?php  }
		...
		<table class="footable table table-stripped" data-page-size="100" data-filter=#filter>
			...
			<tbody>
				<tr>
					<td colspan="3">Upcoming Appointments1</td>
				</tr>
				<?php 
				...
				<tr>
					...
					<td>
						...
						<?php } ?><!--data-toggle="modal" data-target="#myModal<?php echo $list['appoint_trans_id']; ?>"-->
						<div class="modal inmodal" id="myModal<?php echo $list['appoint_trans_id']; ?>" tabindex="-1" role="dialog" aria-hidden="true">
							<div class="modal-dialog">
								<div class="modal-content animated bounceInRight">
									<form method="post" enctype="multipart/form-data" class="form-horizontal form-label-left"  action="add_details.php"  name="frmAddPatient" id="frmAddPatient">
	...
	<?php  }
		...
		<table class="footable table table-stripped" data-page-size="100" data-filter=#filter>
			...
			<tbody>
				<tr>
					<td colspan="3">Missed Appointments</td>
				</tr>
				<?php 
					
					foreach($appUpcomingMissed as $list)
					{ 
						$utc_slot = $list['app_time'];
						$UTCObj   = new DateTime($utc_slot, new DateTimeZone("UTC"));
						$LocalObj = $UTCObj;
	...
	<table class="footable table table-stripped" data-page-size="100" data-filter=#filter>
		...
		<tbody>
			...
			<tr>
				<td><a href="<?php echo $_SESSION['EMR_URL'].md5($Alllist['patient_id']).'&TID='.md5($Alllist['appoint_trans_id']) . '_' .$Alllist['appoint_trans_id'];?>"><?php echo $Alllist['patient_name']; ?></a></td>
				<td style="min-width:200px;" ><?php echo date('d-m-Y',strtotime($Alllist['Visiting_date']))." | ".$time_slot; ?></td>
				<td>
					<div class="btn-group pull-right">
						<?php 
				...
				<div id="before-search">
					...
					<input type="hidden" id="hosp_id"       name="hosp_id"    	  value="<?php echo $_SESSION['login_hosp_id']; ?>" />
						...
						<div class="form-group">
							...
							<div class="col-sm-4"><input type="text" name="se_pat_name" required="required" class="form-control" required="" autocomplete="off" ></div>
							<?php	if($getDocSpec[0]['spec_id']=="32"){ ?>
							<label class="col-sm-2 control-label">DOB</label>
							<div class="col-sm-4">									
								<input id="dateadded" name="date_birth" type="text" value="" placeholder="" class="form-control" autocomplete="off" >
							</div>
							<?php }  else{ ?>
							<label class="col-sm-2 control-label">Age </label>
							<div class="col-sm-4"><input type="number" name="se_pat_age" class="form-control" autocomplete="off" ></div>
							<?php } ?>								
						</div>
						<div class="form-group">
						...
						<div class="form-group">
							...
							<div class="col-sm-4"><input type="number" class="form-control" name="se_phone_no" required="required" maxlength="10"  placeholder="10 digit mobile no." autocomplete="off" /></div>
						...
						<div id="beforeLoad">
							...
							<div class="form-group">
								...
								<div class="col-sm-10">
									<select data-placeholder="Choose a Country..." class="chosen-select" name="se_country"  tabindex="2" onchange="return getState(this.value); ">
						...
						<div class="content collapse" id="collapseRow">
							<div class="form-group" style=" padding-top:10px;">
								...
								<div class="col-sm-8">
									<select class="chosen-select" name="reference_from" id="reference_from" >
								...
								<div class="col-sm-2"><a href="#" class="btn btn-default btn-circle" type="button" title="Click here to add new reference doctor" data-toggle="modal" data-target="#myModal1"><i class="fa fa-plus"></i>
									</a>
								</div>
							</div>
							<div class="form-group">
								<label class="col-sm-2 control-label">Referring Hospital </label>
								<div class="col-sm-8">
									<select class="form-control" name="reference_hosp" id="reference_hosp" onchange="return getrefDocDet(this.value); ">
										<option value="">Select </option>
								...
								<div class="col-sm-2"><a href="#" class="btn btn-default btn-circle" type="button" title="Click here to add new Referring hospital" data-toggle="modal" data-target="#myModal2"><i class="fa fa-plus"></i>
									</a>
								</div>
								<script type="text/javascript">
							...
							<div class="form-group">
								<label class="col-sm-2 control-label">Referring Doctor </label>
								<div class="col-sm-8">
									<select class="form-control" name="refering_doc" id="refering_doc" >
										<option value="">Select </option>
								...
								<div class="col-sm-2"><a href="#" class="btn btn-default btn-circle" type="button" title="Click here to add new Referring doctor" data-toggle="modal" data-target="#myModal3"><i class="fa fa-plus"></i>
									</a>
								</div>
							</div>
							<div class="form-group">
								<label class="col-sm-2 control-label">Referral Note</label>
								<div class="col-sm-4">
						...
						<div class="form-group">
							...
							<div class="col-sm-8 m-t">
								<dl>
									<dt><label> <input type="checkbox" class="i-checks" name="chkReceipt" value="1"> Would you like to send payment receipt to patient via SMS?</label></dt>
									<br> <br>
								</dl>
							</div>
						</div>
						...
						<form method="post" enctype="multipart/form-data" class="form-horizontal form-label-left"  action="add_details.php"  name="frmAddReferred">
							...
							<div class="modal-body">
								...
								<div class="form-group">
									...
									<select data-placeholder="Choose a Specialization..." class="form-control" name="slctSpec" tabindex="3">
