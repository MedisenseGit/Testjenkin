...
function timeAgo($datefrom,$dateto=-1)
	{
		...
		else if($difference >= 60*60*24*7 && $difference <
		60*60*24*30)
		{
			$interval = "ww";
		}
...
{
	$query = mysqli_query("SELECT * FROM event_visitors_tab WHERE going='$partner_id' and event_id='$event_id' LIMIT 1");
...
{
	$query = mysqli_query("SELECT * FROM event_visitors_tab WHERE event_id='$event_id' and going!=0");
	$num_goings = mysqli_num_rows($query);
	return $num_goings;
}

function check_maybe($partner_id,$event_id) 
{
	$query = mysqli_query("SELECT * FROM event_visitors_tab WHERE maybe='$partner_id' and event_id='$event_id' LIMIT 1");
	$maybe = mysqli_num_rows($query);
	return $maybe;
}

function maybe($event_id)
{
	$query = mysqli_query("SELECT * FROM event_visitors_tab WHERE event_id='$event_id' and maybe!=0");
	$num_maybe = mysqli_num_rows($query);
	return $num_maybe;
}

function check_cannot($partner_id,$event_id) 
{
	$query = mysqli_query("SELECT * FROM event_visitors_tab WHERE cannotgo='$partner_id' and event_id='$event_id' LIMIT 1");
	$cannot = mysqli_num_rows($query);
	return $cannot;
}

function cannot($event_id) 
{
	$query = mysqli_query("SELECT * FROM event_visitors_tab WHERE event_id='$event_id' and cannotgo!=0");
	$num_cannot = mysqli_num_rows($query);
	return $num_cannot;
}

function check_bloglike($user_id,$cat_type,$post_id,$user_type) 
{
	$query = mysqli_query("SELECT * FROM home_post_like WHERE likes='$user_id' and category_id='$post_id' and category_type='$cat_type' and  user_type='$user_type' LIMIT 1");
	$numlikes = mysqli_num_rows($query);
	return $numlikes;
}

function bloglike($post_id,$cat_type)
{
	$query = mysqli_query("SELECT * FROM home_post_like WHERE category_id='$post_id' and category_type='$cat_type'");
	$num_like = mysqli_num_rows($query);
	return $num_like;
}
