...
if(isset($_GET['irisid']) && !empty($_GET['irisid'])){
	...
	if(is_numeric($params[0]) == false){
		...
		$check_exists_iris = mysqlSelect("*","examination_ophthal_iris","iris_name='".$params[0]."' and doc_id='".$admin_id."' and doc_type='1' ","","","","");
		if(COUNT($check_exists_iris)==0){ //To prevent double entry
			$insert_chamber=mysqlInsert('examination_ophthal_iris',$arrFileds_master,$arrValues_master);
			$iris_id = $insert_chamber;
			
			$arrFileds[]='iris';
			$arrValues[]=$iris_id;
			$insert_chamber=mysqlInsert('doc_patient_iris_active',$arrFileds,$arrValues);
	...
	} else
	{
		...
		if(isset($_GET['eye_type']) && isset($_GET['iris_status'])) {
		    ...
		    $upd_chamber = mysqlUpdate("examination_ophthal_iris", array($upd_field), array($upd_val), $upd_where);
			
			if($upd_val == 0) {
				if($Edit_Session == 1){
				mysqlDelete('doc_patient_iris_active',"episode_id='".$Episode_Id."' and iris='".$iris_id."' and doc_id='".$admin_id."' and doc_type='1' and eye_side='".$_GET['eye_type']."' and patient_id='".$_GET['patientid']."' and status='0'");
				}
				else
				{
				mysqlDelete('doc_patient_iris_active',"iris='".$iris_id."' and doc_id='".$admin_id."' and doc_type='1' and eye_side='".$_GET['eye_type']."' and patient_id='".$_GET['patientid']."' and status='1'");
				}
			}
			else {
				$insert_chambert=mysqlInsert('doc_patient_iris_active',$arrFileds,$arrValues);
			}
