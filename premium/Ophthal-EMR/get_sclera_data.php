...
if(isset($_GET['scleraid']) && !empty($_GET['scleraid'])){
	...
	if(is_numeric($params[0]) == false){
		...
		$check_exists_sclera = mysqlSelect("*","examination_ophthal_sclera","scelra_name='".$params[0]."' and doc_id='".$admin_id."' and doc_type='1' ","","","","");
		if(COUNT($check_exists_sclera)==0){ //To prevent double entry
			$insert_sclera=mysqlInsert('examination_ophthal_sclera',$arrFileds_sclera,$arrValues_sclera);
			$sclera_id = $insert_sclera;
			
			$arrFileds[]='sclera';
			$arrValues[]=$sclera_id;
			$insert_lids=mysqlInsert('doc_patient_sclera_active',$arrFileds,$arrValues);
	...
	} else
	{
		...
		if(isset($_GET['eye_type']) && isset($_GET['sclera_status'])) {
		    ...
		    $upd_lid = mysqlUpdate("examination_ophthal_sclera", array($upd_field), array($upd_val), $upd_where);
			
			if($upd_val == 0) {
				if($Edit_Session == 1){
				mysqlDelete('doc_patient_sclera_active',"episode_id='".$Episode_Id."' and sclera='".$sclera_id."' and doc_id='".$admin_id."' and doc_type='1' and eye_side='".$_GET['eye_type']."' and patient_id='".$_GET['patientid']."' and status='0'");
				}
				else
				{
				mysqlDelete('doc_patient_sclera_active',"sclera='".$sclera_id."' and doc_id='".$admin_id."' and doc_type='1' and eye_side='".$_GET['eye_type']."' and patient_id='".$_GET['patientid']."' and status='1'");
				}
			}
			else {
				$insert_lids=mysqlInsert('doc_patient_sclera_active',$arrFileds,$arrValues);
