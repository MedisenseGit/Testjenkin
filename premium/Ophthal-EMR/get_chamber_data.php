...
if(isset($_GET['chamberid']) && !empty($_GET['chamberid'])){
	...
	if(is_numeric($params[0]) == false){
		...
		$check_exists_chamber = mysqlSelect("*","examination_ophthal_chamber","chamber_name='".$params[0]."' and doc_id='".$admin_id."' and doc_type='1' ","","","","");
		if(COUNT($check_exists_chamber)==0){ //To prevent double entry
			$insert_chamber=mysqlInsert('examination_ophthal_chamber',$arrFileds_master,$arrValues_master);
			$chamber_id = $insert_chamber; //Get Patient Id
			
			$arrFileds[]='chamber';
			$arrValues[]=$chamber_id;
			$insert_chamber=mysqlInsert('doc_patient_anterior_chamber_active',$arrFileds,$arrValues);
	...
	} else
	{
		...
		if(isset($_GET['eye_type']) && isset($_GET['chamber_status'])) {
		    ...
		    $upd_chamber = mysqlUpdate("examination_ophthal_chamber", array($upd_field), array($upd_val), $upd_where);
			
			if($upd_val == 0) {
				if($Edit_Session == 1){
				mysqlDelete('doc_patient_anterior_chamber_active',"episode_id='".$Episode_Id."' and chamber='".$chamber_id."' and doc_id='".$admin_id."' and doc_type='1' and eye_side='".$_GET['eye_type']."' and patient_id='".$_GET['patientid']."' and status='0'");
				}
				else
				{
				mysqlDelete('doc_patient_anterior_chamber_active',"chamber='".$chamber_id."' and doc_id='".$admin_id."' and doc_type='1' and eye_side='".$_GET['eye_type']."' and patient_id='".$_GET['patientid']."' and status='1'");
				}
			}
			else {
				$insert_chambert=mysqlInsert('doc_patient_anterior_chamber_active',$arrFileds,$arrValues);
			}
