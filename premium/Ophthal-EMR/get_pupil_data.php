...
if(isset($_GET['pupilid']) && !empty($_GET['pupilid'])){
	...
	if(is_numeric($params[0]) == false){
		...
		$check_exists_pupil = $objQuery->mysqlSelect("*","examination_ophthal_pupil","pupil_name='".$params[0]."' and doc_id='".$admin_id."' and doc_type='1' ","","","","");
		if(COUNT($check_exists_pupil)==0){ //To prevent double entry
			...
			$pupil_id = $insert_pupil;
			
			$arrFileds[]='pupil';
			$arrValues[]=$pupil_id;
			$insert_chamber=mysqlInsert('doc_patient_pupil_active',$arrFileds,$arrValues);
	...
	} else
	{
		...
		if(isset($_GET['eye_type']) && isset($_GET['pupil_status'])) {
		    ...
		    $upd_chamber = mysqlUpdate("examination_ophthal_pupil", array($upd_field), array($upd_val), $upd_where);
			
			if($upd_val == 0) {
				if($Edit_Session == 1){
				mysqlDelete('doc_patient_pupil_active',"episode_id='".$Episode_Id."' and pupil='".$pupil_id."' and doc_id='".$admin_id."' and doc_type='1' and eye_side='".$_GET['eye_type']."' and patient_id='".$_GET['patientid']."' and status='0'");
				}
				else
				{
				mysqlDelete('doc_patient_pupil_active',"pupil='".$pupil_id."' and doc_id='".$admin_id."' and doc_type='1' and eye_side='".$_GET['eye_type']."' and patient_id='".$_GET['patientid']."' and status='1'");
				}
			}
			else {
				$insert_chambert=mysqlInsert('doc_patient_pupil_active',$arrFileds,$arrValues);
			}
