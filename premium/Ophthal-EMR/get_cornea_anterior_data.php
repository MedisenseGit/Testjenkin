...
if(isset($_GET['cornea_antid']) && !empty($_GET['cornea_antid'])){
	...
	if(is_numeric($params[0]) == false){
		...
		$check_exists_corneaant = mysqlSelect("*","examination_ophthal_cornea_anterior","cornea_ant_name='".$params[0]."' and doc_id='".$admin_id."' and doc_type='1' ","","","","");
		if(COUNT($check_exists_corneaant)==0){ //To prevent double entry
			$insert_corneaAnt=mysqlInsert('examination_ophthal_cornea_anterior',$arrFileds_master,$arrValues_master);
			$cornea_ant_id = $insert_corneaAnt;
			
			$arrFileds[]='cornea_ant';
			$arrValues[]=$cornea_ant_id;
			$insert_lids=mysqlInsert('doc_patient_cornea_ant_active',$arrFileds,$arrValues);
	...
	} else
	{
		...
		if(isset($_GET['eye_type']) && isset($_GET['cornea_ant_status'])) {
		    ...
		    $upd_lid = mysqlUpdate("examination_ophthal_cornea_anterior", array($upd_field), array($upd_val), $upd_where);
			
			if($upd_val == 0) {
				if($Edit_Session == 1){
				mysqlDelete('doc_patient_cornea_ant_active',"episode_id='".$Episode_Id."' and cornea_ant='".$cornea_ant_id."' and doc_id='".$admin_id."' and doc_type='1' and eye_side='".$_GET['eye_type']."' and patient_id='".$_GET['patientid']."' and status='0'");
				}
				else
				{
				mysqlDelete('doc_patient_cornea_ant_active',"cornea_ant='".$cornea_ant_id."' and doc_id='".$admin_id."' and doc_type='1' and eye_side='".$_GET['eye_type']."' and patient_id='".$_GET['patientid']."' and status='1'");
				}
			}
			else {
				$insert_lids=mysqlInsert('doc_patient_cornea_ant_active',$arrFileds,$arrValues);
