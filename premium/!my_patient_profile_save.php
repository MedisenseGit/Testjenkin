...
if(isset($_GET['suffersincedetail']) && !empty($_GET['suffersincedetail'])){
	...
	$update_diagnodetail=$objQuery->mysqlUpdate('doc_patient_episodes',$arrFileds_diagno,$arrValues_diagno,"md5(episode_id) = '".$_GET['episodeid']."'");
...
if(isset($_GET['treatmentdetail']) && !empty($_GET['treatmentdetail'])){
	...
	$update_treatmentdetail=$objQuery->mysqlUpdate('doc_patient_episodes',$arrFileds_diagno,$arrValues_diagno,"md5(episode_id) = '".$_GET['episodeid']."'");
...
if(isset($_GET['prescnote']) && !empty($_GET['prescnote'])){
	...
	$update_prescnote=$objQuery->mysqlUpdate('doc_patient_episodes',$arrFileds_presc,$arrValues_presc,"md5(episode_id) = '".$_GET['episodeid']."'");
...
if(isset($_POST['updateDiagnoInvestigation']))
{
	while(list($key_invest, $value_invest) = each($_POST['investigation_id']))
	{
		...
		$insert_trend_analysis= $objQuery->mysqlInsert('trend_analysis',$arrFieldTrend,$arrValueTrend);
				...
				$getInvestDetails= $objQuery->mysqlSelect("*","patient_temp_investigation","pti_id = '".$_POST['investigation_id']."'","","","","");
				$getDocDetails= $objQuery->mysqlSelect("ref_name,contact_num","referal","ref_id='".$_POST['doc_id']."'","","","","");
				$getDiagnoDetails= $objQuery->mysqlSelect("diagnosis_name","Diagnostic_center","diagnostic_id='".$_POST['diagno_id']."'","","","","");
				...
				$arrValueStatus[]="2";
				$update_status=$objQuery->mysqlUpdate('diagnostic_referrals',$arrFieldStatus,$arrValueStatus,"diagnostic_id='".$_POST['diagno_id']."' and patient_id = '".$getInvestDetails[0]['patient_id']."' and episode_id='".$getInvestDetails[0]['episode_id']."'");
...
//SAVE PATIENT	
	...
	if(isset($_POST['changeDate'])){
		...
		$userupdate=$objQuery->mysqlUpdate('doc_patient_episodes',$arrFields,$arrValues, "md5(episode_id) = '".$_POST['episode_id']."'");
	...
	if(isset($_POST['updatePatient'])){
		...
		//Update Drug Abuse -' doc_patient_drug_active' table
				...
				$update_icd=$objQuery->mysqlUpdate('doc_patient_drug_active',$arrFieldsDrugAbuse,$arrValuesDrugAbuse,"patient_id = '".$_POST['patient_id']."' and doc_id='".$admin_id."' and doc_type='1' and status='1'");
				
				//Update Family History -' doc_patient_drug_active' table
				
				$arrFieldsFamilyHistory[] = 'status';
				$arrValuesFamilyHistory[] = "0";
				$update_icd=$objQuery->mysqlUpdate('doc_patient_family_history_active',$arrFieldsFamilyHistory,$arrValuesFamilyHistory,"patient_id = '".$_POST['patient_id']."' and doc_id='".$admin_id."' and doc_type='1' and status='1'");
	...
	if(isset($_POST['update_patient_print'])){
				...
				$update_episode=$objQuery->mysqlUpdate('doc_patient_episodes',$arrFieldsEPISODE,$arrValuesEPISODE,"episode_id = '".$_POST['episode_id']."'");
		...
		/* Save Examination template details Starts here */
				...
				if ($chkExamSaveTemplate == 1)
				{
					...
					$getChosenExam= $objQuery->mysqlSelect("*","doc_patient_examination_active","doc_id='".$admin_id."' and doc_type='1' and episode_id='".$_POST['episode_id']."'","","","","");
				...
				if ($chkInvestSaveTemplate == 1)
				{
					...
					$getChosenInvset= $objQuery->mysqlSelect("*","patient_temp_investigation","doc_id='".$admin_id."' and doc_type='1' and episode_id='".$_POST['episode_id']."'","","","","");
				...
				//Save for Appointment Payment Transaction
					...
					$chkPatientAppTab = $objQuery->mysqlSelect("*","appointment_token_system","patient_id='".$_POST['patient_id']."' and doc_id='".$admin_id."' and doc_type='1' and hosp_id='".$_SESSION['login_hosp_id']."' and app_date='".date('Y-m-d')."'","","","","");
					if(count($chkPatientAppTab)>0)
					{
						...
						$update_appoint_trans=$objQuery->mysqlUpdate('appointment_transaction_detail',$arrFieldsAppTransStatus,$arrValuesAppTransStatus,"Visiting_date = '".date('Y-m-d')."' and patient_id='".$_POST['patient_id']."'and pref_doc='".$admin_id."' and hosp_id='".$_SESSION['login_hosp_id']."'");	
...
if(isset($_POST['updateInvestigation']))
{
	while(list($key_invest, $value_invest) = each($_POST['investigation_id']))
	{
		...
		$arrFieldTrend[]='patient_type';
		$arrValueTrend[]="1";
		$checkTrend= $objQuery->mysqlSelect("*","trend_analysis","date_added='".$_POST['date_added']."' and patient_id='".$_POST['patient_id']."' and patient_type='1'","","","","");
		if(count($checkTrend)>0)
		{
			$update_trend=$objQuery->mysqlUpdate('trend_analysis',$arrFieldTrend,$arrValueTrend,"date_added='".$_POST['date_added']."' and patient_id = '".$_POST['patient_id']."' and patient_type='1'");
		}
		else
		{
		$insert_trend_analysis= $objQuery->mysqlInsert('trend_analysis',$arrFieldTrend,$arrValueTrend);
...
{
	$objQuery->mysqlDelete('doc_my_patient_reports',"md5(report_id)='".$_GET['reportid']."'");	
}
