...
<html>
	...
	<body>
				...
				<div class="wrapper wrapper-content animated fadeInRight">
					<div class="row">
						...
						<div class="alert alert-danger alert-dismissable" style="display:none" id="appointTime">
							...
							<strong>
								<i class="fa fa-warning"></i> Please update appointment timings 
							</strong>
						</div>
							...
							<div class="ibox float-e-margins">
										...
										<div class="col-sm-12">
											<select data-placeholder="Choose doctor..." class="chosen-select" tabindex="2" onchange="return saveDoctorId(this.value);">
												...
												<option value="<?php echo stripslashes($DocList['Ref_Id']); ?>" <?php if($_SESSION['appointTypeDoc']==stripslashes($DocList['Ref_Id'])){?>selected<?php }?> />
													<?php echo stripslashes($DocList['Doc_name']).", ".stripslashes($DocList['Hosp_name']);?>
												</option>
												<?php 
													$i++;
													}?> 
								...
								<div class="ibox-content" id="before-status">
									...
									<table class="footable table table-stripped" data-page-size="100" data-filter=#filter>
										...
										<tbody>
											...
											<tr>
											<td><?php if($Todaylist['token_no']!="555") 
											{ 
												echo "<button class='btn btn-success btn-circle' type='button'>".$Todaylist['token_no']."</button>";
											} 
											else
											{ 
												echo "<button class='btn btn-primary btn-xs' type='button'>Online</button>"; 
											} 
											?>
											</td>
											<?php 
											$getDoctorName= mysqlSelect("ref_id as Ref_Id,ref_name as Doc_name","referal","ref_id='".$Todaylist['doc_id']."'","ref_name asc","","","");
											...
											<?php 
											if($Todaylist['status']!="Cancelled")
											{ 
											?>
											<ul class="dropdown-menu">
												<?php
												 foreach($status_val as $key=>$value)
												{
												?>
											...
											if($Todaylist['status']!="Cancelled")
											{ 
											?><a href="Reschedule?a=<?php echo $Todaylist['appoint_trans_id']; ?>"><i class="fa fa-edit"></i></a> 
											</center>
											<?php 
											} 
											?>
									...
									<table class="footable table table-stripped" data-page-size="100" data-filter=#filter>
										...
										<tbody>
											<tr><td colspan="3">Upcoming Appointments</td></tr>
												<?php foreach($appUpcomingResult as $list){  ?>
											<tr>
												<?php 
												
												$getDoctorName= mysqlSelect("ref_id as Ref_Id,ref_name as Doc_name","referal","ref_id='".$list['doc_id']."'","ref_name asc","","","");
								...
								<table class="footable table table-stripped" data-page-size="100" data-filter=#filter>
									...
									<tbody>
										...
										<?php
										foreach($appointmentResult as $Alllist)
										{ 
										...
										<tr>
											<?php
											$getDoctorName= mysqlSelect("ref_id as Ref_Id,ref_name as Doc_name","referal","ref_id='".$Alllist['pref_doc']."'","ref_name asc","","","");
											?>
											<td><?php echo $getDoctorName[0]['Doc_name']?></td>
											...
											<td>
												<div class="btn-group pull-right">
													<?php 
														...
														?>
													<button data-toggle="dropdown" class="btn <?php echo $btn_type;?> btn-xs dropdown-toggle"><?php 
														echo $Alllist['pay_status']; 
													?>
													<span class="caret"></span></button>
													<?php 
													if($Alllist['pay_status']!="Cancelled")
													{ 
													?>
													<ul class="dropdown-menu">
														<?php 
											...
											{ 
											?>	
											<a href="Reschedule?a=<?php echo $Alllist['appoint_trans_id']; ?>"><i class="fa fa-edit"></i></a> </center>
											<?php
											} 
											?>
											...
											<div class="form-group">
												...
												<div class="col-sm-4">
													<select data-placeholder="Choose Preferred Time..." class="form-control chkTime" name="check_time"  id="check_time<?php echo $Alllist['appoint_trans_id']; ?>" tabindex="2" required="required">
													</select>
												</div>
								...
								<div id="tab-1" class="tab-pane active">
									<div class="panel-body">
										...
										<div class="alert alert-danger alert-dismissable" id="timeSlot" style="display:none">
											<button aria-hidden="true" data-dismiss="alert" class="close" type="button">×</button>
											<strong>
												<i class="fa fa-alert"></i> No appointment slot available <!--<a href="Set-Appointment">View Settings</a>-->
											</strong>
										</div>
										<?php// }?>
										...
										<div class="form-horizontal">
											...
											<div class="form-group" id="doctor_section" >
												...
												<div class="col-sm-10">
													<select data-placeholder="Choose doctor..." class="chosen-select" name="doc_id" id="doc_id" required="required"  tabindex="2" onchange="return getDocDatefun(this.value);">
														<option value="" selected>Choose Doctor</option>
														<?php 
															$getDoctor= mysqlSelect("a.ref_id as Ref_Id,a.ref_name as Doc_name,c.hosp_name as Hosp_name,d.spec_name as Department","referal as a inner join doctor_hosp as b on a.ref_id=b.doc_id inner join hosp_tab as c on c.hosp_id=b.hosp_id inner join doc_specialization as e on e.doc_id=a.ref_id inner join specialization as d on d.spec_id=e.spec_id","c.hosp_id='".$admin_id."'","a.ref_name asc","","","");
																$i=30;
										...
										<!--	<div class="row well well-lg">
												...
												<div class="scrolling-wrapper">	
													...
													for($i=0; $i<=20; $i++)
													{
														...
														$date_1 = new DateTime($current_date);
														$current_time_stamp=$date_1->format("U"); 
														foreach($getDocDates as $daylist)
														{ 
															foreach($getDocDays as $daynamelist)
															{
																if($daylist['DayId'] == $daynamelist['day_id'])
																{
																	$getDayName = $daynamelist['da_name'];
																}
															}
															if(date('D', $date)== $getDayName)
															{ 
															?>
												...
												<div class="modal-content animated bounceInRight">
													...
													<form method="post" enctype="multipart/form-data" class="form-horizontal form-label-left"  action="add_details.php"  name="frmAddReferred">
														...
														<div class="modal-body">
															<div class="form-group">
																<label>Name</label> 
																<input type="text" name="referral_name" required value="" class="form-control">
															</div>
															<div class="form-group">
																<label>Email</label>
																<input type="email" name="referral_email" value="" class="form-control">
															</div>
															<div class="form-group">
																<label>Mobile</label> 
																<input type="text" name="referral_mobile" value="" class="form-control" title="Mobile Number must be 10 digits">
															</div>
															
															<div class="form-group">
																<label>Address</label>
																<input type="text" name="referral_address" value="<?php echo $getRefDet[0]['ref_address']; ?>" class="form-control">
															</div>
															
															<div class="form-group">
																<label>City</label>
																<input type="text" name="referral_city" value="<?php echo $getRefDet[0]['doc_city']; ?>" class="form-control">
															</div>
												...
												<div class="modal-content animated bounceInRight">
													...
													<form method="post" enctype="multipart/form-data" class="form-horizontal form-label-left"  action="add_details.php"  name="frmAddReferred">
														...
														<div class="modal-body">
															<div class="form-group">
																<label>Name</label>
																<input type="text" name="hos_name" required value="" class="form-control">
															</div>
															<div class="form-group">
																<label>Email</label> 
		...
		<!-- Page-Level Scripts -->
		<script>
		/*function getDateFun(date_result,day)
		{
			var day_id;
			if(day=="Sun")
			{
			...
			$('#se_date').val(date_result);
			var docId 	= '3727';		//<?php echo $docDetailsArray->ref_id; ?>;
			var hosp_id	='1093';	//$('#se_depart :selected').val();
			
			$.ajax({
				type: "POST",
				url: "get_doc_slot.php",
				data:{"doc_id":docId,"hosp_id":hosp_id,"day_id":day_id},
				success: function(data)
				{
					$("#slot").html(data);
				}
		...
		function getSlotFun(timeslot)
		{
			$('#se_time').val(timeslot);
		}*/
