...
$getpatient = mysqlSelect("*","doc_my_patient","patient_id='".$patientid."'","","","","");
if(isset($_GET['drugid']) && !empty($_GET['drugid'])){
	
	$params     = explode("-", $_GET['drugid']);
	...
	if(is_numeric($params[0]) == false){
		...
		$arrValues_drug[]=$getpatient[0]['doc_id'];
	...
	} else
	{
		...
		$insert_drug=mysqlInsert('doc_patient_drug_active',$arrFileds,$arrValues);
				...
				$check_drug = mysqlSelect("*","doctor_frequent_drug_abuse","drug_abuse_id='".$drug_id."' and doc_id='".$getpatient[0]['doc_id']."' and doc_type='1'","","","","");
					...
					if(count($check_drug)>0){
						...
						$arrValuesSYMPFREQ[] = $getpatient[0]['doc_id'];
...
$getDrugRes= mysqlSelect("b.drug_abuse as drug_abuse,a.drug_active_id as drug_active_id","doc_patient_drug_active as a left join drug_abuse_auto as b on a.drug_abuse_id=b.drug_abuse_id","a.doc_id='".$getpatient[0]['doc_id']."' and a.patient_id='".$patientid."' and a.doc_type='1' and a.status='0'","","","","");
