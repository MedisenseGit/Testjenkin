...
if(isset($_GET['transid']) && !empty($_GET['transid'])){
	...
	if($_GET['statusId']==3){
		...
		$longurl = "/SendRequestLink/?d=".md5($getPatDetails[0]['doc_id'])."&hid=".md5($admin_id);
		//Get Shorten Url
		$getUrl= get_shorturl($longurl);
		
		$txtMob = $getPatDetails[0]['Mobile_no'];					
		$recieptmsg= "Your appointment with ".$getPatDetails[0]['doc_name']." has been cancelled. Pls book again using this ".$getUrl." Thanks ".$getCompanyProfile[0]['hosp_name'];
