            ...
            <div class="row wrapper border-bottom white-bg page-heading">
                <div class="col-lg-10">
                    ...
                    <ol class="breadcrumb">
                        ...
                        <li class="active">
                            <strong>Corporate Appointments</strong>
                        </li>
                    </ol>
                </div>
                <div class="col-lg-2">
                        ...
                        <div class="tab-content">
                            <div id="tab-1" class="tab-pane active">
								...
								<hr>
									...
									<form enctype="multipart/form-data" method="post" action="add_details.php"  name="frmAddPatient" id="frmAddPatient1" autocomplete="off" >

									<!--<div id="date-time-section"></div>-->
								<div class="form-group"><label class="col-sm-2 control-label">Subscribe Company <span class="required">*</span></label>
                                    ...
                                    <div class="col-sm-10"><select data-placeholder="Choose doctor..." class="form-control form-control-lg" name="doc_id" id="doc_id"   tabindex="2" onchange="return getDocDate(this.value);" class="required" required="">
											<option value="" selected>Choose Doctor</option>
												<?php 
												$getDoctor= mysqlSelect("a.ref_id as Ref_Id,a.ref_name as Doc_name,c.hosp_name as Hosp_name,d.spec_name as Department","referal as a inner join doctor_hosp as b on a.ref_id=b.doc_id inner join hosp_tab as c on c.hosp_id=b.hosp_id inner join doc_specialization as e on e.doc_id=a.ref_id inner join specialization as d on d.spec_id=e.spec_id","c.hosp_id='".$admin_id."'","a.ref_name asc","","","");
													$i=30;
	                          ...
	                          <!--  <div class="col-sm-10">
	                               	<select name="select"   name="doc_id" id="doc_id"   tabindex="2" onchange="return getDocDate(this.value);" class="required" required="">
							   ...
							   <div class="form-group" id="date_section">
								...
								<div class="form-group">
                                    ...
                                    <div class="col-sm-4"><input type="text" name="se_pat_name" class="form-control" required=""></div>
								...
								<button type="submit" name="corporate_appointment" id="corporate_appointment" <?php //if(count($chkDocTimeSlot)==0){ echo "disabled"; }?> class="btn btn-primary block full-width m-b ">BOOK APPOINTMENT</button>
								</div>
								</div>
								</form>
								</div>
...
$(document).ready(function(){ //alert("hi");
	...
	// $get_PatientDetails = mysqlSelect("e.patient_id,e.patient_name,e.patient_mob","doc_my_patient as e inner join referal as a on a.ref_id=e.doc_id inner join doctor_hosp as b on a.ref_id=b.doc_id inner join hosp_tab as c on c.hosp_id=b.hosp_id inner join specialization as d on d.spec_id=a.doc_spec","c.hosp_id='".$admin_id."'","","","","");
		...
		//$subscribe_company_name1 =$_SESSION[$get_PatientDetails['employee_name']];  
   ...
   //          $(function () {
			//   //$('#subscribe_company_name').val("fgg"); 
			//   alert("hi");
			//   $('#subscribe_company_name').val("fgg").change();
			// });
...
$('#onlineConsult').click(function() {
	...
	<!-- validation of select on the -->
	<script type="text/javascript">
	jQuery(function() { //alert("hi");  
    jQuery('.error').hide(); // Hide Warning Label. 
    jQuery("input[name=direct_appointment]").on("click", function() {
        var returnvalue;// alert("hi");
        if(jQuery("select[name=se_sub_comp]").val() == 0) {
            jQuery("label#select_error").show(); // show Warning 
            jQuery("select#se_sub_comp").focus();  // Focus the select box      
            returnvalue=false;   
        }
        return returnvalue;
    });
...
<script>
  $('#frmAddPatient1').validate({ // initialize the plugin

            rules: {
          se_sub_comp:{ required: true},
         }
      });
