...
if(!empty($_POST["template_id"])) {
	$template_details = $objQuery->mysqlSelect("*","patient_episode_prescription_template_details","template_id='".$template_id."' and admin_id = '". $admin_id ."' ","","","","");
	$strResponse = array();
	//$prescription_seq = 0;
	while (list($template_detail_key, $template_detail_val) = each($template_details))
	{
		...
		//$strResponse .= '<tr class="link1" id="prescription_del_'. $prescription_seq .'_row">';
			$strResponse .= '<tr class="link1" id="prescription_del_'. $template_id.'_'.$prescription_seq .'_row">';
						...
						$strResponse .=  '</td>';
						$strResponse .=  '<td class="fields">';
							$strResponse .=  '<button id="prescription_del_'. $template_id.'_'.$prescription_seq .'" class="btn btn-sm btn-danger pull-right m-xs delbutton" ><i class="fa fa-trash"></i> Delete</button>';
						$strResponse .=  '</td>';
					$strResponse .=  '</tr>';
	}
}
