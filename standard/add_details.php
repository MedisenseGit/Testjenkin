...
if(isset($_POST['request_appointment'])){
	...
	$chkDate = date('Y-m-d',strtotime($Cur_Date));
	$getSource = $objQuery->mysqlSelect("a.source_id as source_id,b.partner_name as partner_name,b.Email_id as Email_id","source_list as a left join our_partners as b on a.partner_id=b.partner_id","a.partner_id='".$admin_id."'" ,"","","","");
	...
	// Create Patient Here
		...
		$insertchat=$objQuery->mysqlInsert('chat_notification',$arrFields_chat,$arrValues_chat);
				...
				$get_department = $objQuery->mysqlSelect('spec_name','specialization',"spec_id='".$docspec."'");
				...
				$docmsg="Dear Doctor, ".$getPatInfo[0]['patient_name']."( Ph: ".$getPatInfo[0]['patient_mob']." )has expressed interest to meet you in person. For more info please login into your Pixel Contributor dash board or email . Thanks";
				if(!empty($pro_contact)){
				$mobile = $pro_contact;
				...
				//Patient Info EMAIL notification Sent to Doctor
					if(!empty($docmail)){
						...
						$prefDate=date('d-M-Y',strtotime($chkInDate));
						...
						$url .= "&prefDate=" . urlencode($prefDate);
						...
						$url .= "&partnername=" . urlencode($getSource[0]['partner_name']);
						$url .= "&replymail=" . urlencode($getSource[0]['Email_id']);	
						$url .= "&docDept=" . urlencode($get_department[0]['spec_name']);						
						send_mail($url);	
					}
...
if(isset($_POST['ref_appointment'])){
	...
	$transid=time();
		...
		//Insert to new_hospvisitor_details table
				...
				$arrValues[] = date('Y-m-d',strtotime($chkInDate));;
	...
	$get_pro = $objQuery->mysqlSelect('partner_id,cont_num1,Email_id,contact_person,partner_name,specialisation','our_partners',"partner_id='".$admin_id."'");
	$get_department = $objQuery->mysqlSelect('spec_name','specialization',"spec_id='".$get_pro[0]['specialisation']."'");
	$msg="Appointment Confirmed, TransactionID ". $transid . " | Patient Name: ". $txtName . " | Doctor: ".$get_pro[0]['contact_person']." | Date & Time: ".$prefDate." | ".$getTime[0]['Timing']." Thanks ".$get_pro[0]['partner_name'];
	send_msg($txtMob,$msg);
	...
	//Patient Info EMAIL notification Sent to Doctor
		...
		$PatAddress=$getPatInfo[0]['patient_addrs'].",<br>".$getPatInfo[0]['patient_loc'].", ".$getPatInfo[0]['pat_state'].", ".$getPatInfo[0]['pat_country'];
		$ccmail=$getPatInfo[0]['patient_email'];
					...
					$url .= "&ccmail=" . urlencode($ccmail);
					$url .= "&partnername=" . urlencode($get_pro[0]['partner_name']);
...
if(isset($_POST['cmdshare'])){
	...
	$weblink="https://medisensecrm.com".$_POST['currenturl'];
	if($_POST['receiverMail']!="")
	{
				$page_url = 'share_post_link.php';
				$paturl = rawurlencode($page_url);
...
if(isset($_POST['cmdReg'])){
	...
	$id= mysql_insert_id();
		...
		header("location:offers.php?s=Events&id=".md5($event_id)."&response=event-success");
}
...
if($_POST['act']=="add-reschedule"){
	...
	$arrValues = array();
		...
		$patientRef=$objQuery->mysqlUpdate('partner_appointment_transaction',$arrFields,$arrValues,"appoint_trans_id='".$_POST['patTransId']."'");
		
		$getInfo1 = $objQuery->mysqlSelect("*","partner_appointment_transaction","appoint_trans_id='".$_POST['patTransId']."'" ,"","","","");	
...
if(isset($_POST['change_password'])){
	...
	if($txtPass==$txtRePass){
		...
		header('location:view-profile?response=password_updated');
	}
	else{
...
if(isset($_POST['postTextSrchCmd'])){
	...
	header("Location:My-Connections?s=".$txtSearch);
...
if(isset($_POST['addActivity']) && !empty($_POST['txtDesc'])){
					...
					//send_msg($mobile,$responsemsg);
					$response=1;	
					header('location:patient-history?response='.$response.'&p='.md5($_POST['patient_id']));					
