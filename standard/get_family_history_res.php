...
if(isset($_GET['historyid']) && !empty($_GET['historyid'])){
	...
	if(is_numeric($params[0]) == false){
		...
		$insert_drugs=$objQuery->mysqlInsert('family_history_auto',$arrFileds_drug,$arrValues_drug);
		$history_id = mysql_insert_id(); //Get Patient Id
	} else
	{
		$history_id = $params[0];
	}
		...
		$insert_drug=$objQuery->mysqlInsert('doc_patient_family_history_active',$arrFileds,$arrValues);
		
				$check_history = $objQuery->mysqlSelect("*","doctor_frequent_family_history","family_history_id='".$history_id."' and doc_id='".$admin_id."' and doc_type='1'","","","","");
					...
					if(count($check_history)>0){
						...
						$update_icd=$objQuery->mysqlUpdate('doctor_frequent_family_history',$arrFieldsSYMPFREQ,$arrValuesSYMPFREQ,"ffh_id = '".$check_history[0]['ffh_id']."'");
					}
					else{
						...
						$arrFieldsSYMPFREQ[] = 'freq_count';
						$arrValuesSYMPFREQ[] = "1";
						$insert_freq_symp=$objQuery->mysqlInsert('doctor_frequent_family_history',$arrFieldsSYMPFREQ,$arrValuesSYMPFREQ);
