...
if(isset($_GET['lidsid']) && !empty($_GET['lidsid'])){
	...
	if(is_numeric($params[0]) == false){
		...
		if(COUNT($check_exists_lids)==0){ //To prevent double entry
			...
			$lidsid = mysql_insert_id(); //Get Patient Id
...
$get_all_lids = $objQuery->mysqlSelect("*","examination_ophthal_lids","(doc_id='".$admin_id."' and doc_type='2' and eye_type='1') or (doc_id='0' and doc_type='0' and eye_type='1') ","lids_name ASC","","","");
$get_all_lidsLE = $objQuery->mysqlSelect("*","examination_ophthal_lids","(doc_id='".$admin_id."' and doc_type='2' and eye_type='2') or (doc_id='0' and doc_type='0' and eye_type='2') ","lids_name ASC","","","");
