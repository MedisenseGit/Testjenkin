...
if(isset($_GET['conid']) && !empty($_GET['conid'])){
	...
	} else
	{
		...
		$insert_symptoms=$objQuery->mysqlInsert('doc_patient_sclera_active',$arrFileds,$arrValues);
				...
				$check_symp = $objQuery->mysqlSelect("*","examination_ophthal_frequent_sclera","sclera_id='".$conjuct_id."' and doc_id='".$admin_id."' and doc_type='2' and eye_side='2'","","","","");
...
?>
						<form method="post" action="my_patient_profile_save.php"  name="frmAddSymp" >
								...
								while(list($key, $value) = each($getConjuct)){
									echo "<input type='hidden' name='lidsID[]' value='" . $value['sclera_id'] . "' /><span class='tag label label-primary m-r'>" . $value['scelra_name'] . "<a data-role='remove' class='text-white del_scleraLE m-l' data-scleraLE-id='".$value['sclera']."'>x</a></span>";
								}
								?>
