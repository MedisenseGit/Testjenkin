...
if(isset($_GET['conid']) && !empty($_GET['conid'])){
	...
	} else
	{
		...
		$check_active_conjuctivaRE = $objQuery->mysqlSelect("*","doc_patient_conjuctiva_active","conjuctiva='".$params[0]."' and doc_id='".$admin_id."' and doc_type='2' and eye_side='1' and patient_id='".$_GET['patientid']."' and status='1'","","","","");
		if(COUNT($check_active_conjuctivaRE)==0){ //To prevent double entry
			$insert_conjuctivaRE=$objQuery->mysqlInsert('doc_patient_conjuctiva_active',$arrFileds,$arrValues);
		}

			/*	$check_symp = $objQuery->mysqlSelect("*","examination_ophthal_frequent_conjuctiva","conjuctiva_id='".$conjuct_id."' and doc_id='".$admin_id."' and doc_type='2' and eye_side='1'","","","","");
...
?>
						<form method="post" action="my_patient_profile_save.php"  name="frmAddSymp" >
								...
								while(list($key, $value) = each($getConjuct)){
									echo "<input type='hidden' name='conjuctID[]' value='" . $value['conjuctiva_id'] . "' /><span class='tag label label-primary m-r'>" . $value['conjuctiva_name'] . "<a data-role='remove' class='text-white del_conjuctivaRE m-l' data-conjuctRE-id='".$value['conjuctiva']."'>x</a></span>";
								}
								?>
