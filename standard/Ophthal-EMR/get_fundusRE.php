...
if(isset($_GET['conid']) && !empty($_GET['conid'])){
	...
	} else
	{
		...
		$insert_symptoms=$objQuery->mysqlInsert('doc_patient_fundus_active',$arrFileds,$arrValues);
				...
				$check_symp = $objQuery->mysqlSelect("*","examination_ophthal_frequent_fundus","fundus_id='".$conjuct_id."' and doc_id='".$admin_id."' and doc_type='2' and eye_side='1'","","","","");
...
?>
						<form method="post" action="my_patient_profile_save.php"  name="frmAddSymp" >
								...
								while(list($key, $value) = each($getConjuct)){
									echo "<input type='hidden' name='lidsID[]' value='" . $value['fundus_id'] . "' /><span class='tag label label-primary m-r'>" . $value['fundus_name'] . "<a data-role='remove' class='text-white del_fundusRE m-l' data-fundusRE-id='".$value['fundus']."'>x</a></span>";
								}
								?>
