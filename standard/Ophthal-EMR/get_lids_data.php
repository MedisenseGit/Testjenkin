...
if(isset($_GET['lidsid']) && !empty($_GET['lidsid']))
{
	...
	} else
	{
		...
		if(isset($_GET['eye_type']) && isset($_GET['lid_status'])) {
		    ...
		    $upd_lid = $objQuery->mysqlUpdate("examination_ophthal_lids", array($upd_field), array($upd_val), $upd_where);

			if($upd_val == 0) {
				if($Edit_Session == 1){
				$objQuery->mysqlDelete('doc_patient_lids_active',"episode_id='".$Episode_Id."' and lids='".$lids_id."' and doc_id='".$admin_id."' and doc_type='2' and eye_type='".$_GET['eye_type']."' and patient_id='".$_GET['patientid']."' and status='0'");
				}
				else
				{
				$objQuery->mysqlDelete('doc_patient_lids_active',"lids='".$lids_id."' and doc_id='".$admin_id."' and doc_type='2' and eye_type='".$_GET['eye_type']."' and patient_id='".$_GET['patientid']."' and status='1'");
				}
			}
			else {
				$insert_lids=$objQuery->mysqlInsert('doc_patient_lids_active',$arrFileds,$arrValues);
...
if(isset($_GET['lidsidLE']) && !empty($_GET['lidsidLE']))
	...
	if(is_numeric($params1[0]) == false){
		...
		$check_exists_lids = $objQuery->mysqlSelect("*","examination_ophthal_lids","lids_name='".$params1[0]."' and doc_id='".$admin_id."' and doc_type='2' ","","","","");
		if(COUNT($check_exists_lids)==0){ //To prevent double entry
			$insert_lids=$objQuery->mysqlInsert('examination_ophthal_lids',$arrFileds_symp,$arrValues_symp);
			$lidsLE_id = mysql_insert_id(); //Get Patient Id
	...
	} else
	{
		...
		$check_active_lidsLE = $objQuery->mysqlSelect("lids_id","doc_patient_lids_active","lids='".$params1[0]."' and doc_id='".$admin_id."' and doc_type='2' and eye_type='2' and patient_id='".$_GET['patientid']."' and status='1'","","","","");
		//print_r("param val");
		//print_r($params1[0]);
			//print_r("count num");
		//print_r($check_active_lidsLE[0]['LID_COUNT']);
		if(COUNT($check_active_lidsLE)==0 || $Edit_Session == 1){ //To prevent double entry
			$insert_lidsLE=$objQuery->mysqlInsert('doc_patient_lids_active',$arrFileds_LE,$arrValues_LE);
		}
		else {
			if($Edit_Session == 1)
			{
			$objQuery->mysqlDelete('doc_patient_lids_active',"episode_id='".$Episode_Id."' and lids='".$params1[0]."' and doc_id='".$admin_id."' and doc_type='2' and eye_type='2' and patient_id='".$_GET['patientid']."' and status='0'");
			}
			else
			{
			$objQuery->mysqlDelete('doc_patient_lids_active',"lids='".$params1[0]."' and doc_id='".$admin_id."' and doc_type='2' and eye_type='2' and patient_id='".$_GET['patientid']."' and status='1'");
			}
		}
