...
if(isset($_GET['sympid']) && !empty($_GET['sympid'])){
	...
	if(is_numeric($params[0]) == false){
		...
		$arrFileds_symp_RE = array();
		$arrValues_symp_RE = array();

		$arrFileds_symp_RE[]='lids_name';
		$arrValues_symp_RE[]=$params[0];
		$arrFileds_symp_RE[]='eye_type';
		$arrValues_symp_RE[]='1';
		$arrFileds_symp_RE[]='doc_id';
		$arrValues_symp_RE[]=$admin_id;
		$arrFileds_symp_RE[]='doc_type';
		$arrValues_symp_RE[]='2';
		...
		if(COUNT($check_exists_lids)==0){ //To prevent double entry
			...
			$insert_lids_RE=$objQuery->mysqlInsert('examination_ophthal_lids',$arrFileds_symp_RE,$arrValues_symp_RE);
		}
	...
	} else
	{
		...
		$check_active_lids = $objQuery->mysqlSelect("*","doc_patient_lids_active","lids='".$params[0]."' and doc_id='".$admin_id."' and doc_type='2' and eye_type='2' and patient_id='".$_GET['patientid']."' and status='1'","","","","");
		if(COUNT($check_active_lids)==0){ //To prevent double entry
			...
			$objQuery->mysqlDelete('doc_patient_lids_active',"lids='".$params[0]."' and doc_id='".$admin_id."' and doc_type='2' and eye_type='2' and patient_id='".$_GET['patientid']."' and status='1'");
		}

				/* $check_symp = $objQuery->mysqlSelect("*","examination_ophthal_frequent_lids","lids_id='".$symp_id."' and doc_id='".$admin_id."' and doc_type='2' and eye_type='2'","","","","");
...
$get_all_lids = $objQuery->mysqlSelect("*","examination_ophthal_lids","(doc_id='".$admin_id."' and doc_type='2' and eye_type='2') or (doc_id='0' and doc_type='0' and eye_type='2') ","lids_name ASC","","","");
...
?>
						<form method="post" action="my_patient_profile_save.php"  name="frmAddSymp" >
								...
								$getSelectedLidsLE= $objQuery->mysqlSelect("b.lids_name as lids_name,a.lids as lids,b.lids_id as lids_id","doc_patient_lids_active as a left join examination_ophthal_lids as b on a.lids=b.lids_id","a.doc_id='".$admin_id."' and a.patient_id='".$patientid."' and a.doc_type='2' and a.eye_type='2' and a.status='1' and a.lids='".$value['lids_id']."'","","","","");
								if(COUNT($getSelectedLidsLE)>0 ) {
									echo "<input type='checkbox' style='width: 20px;height: 20px; vertical-align: bottom;margin-right:5px;' class='i-checks m-l get_lidsLE_prior' name='" . $value['lids_id'] . "' value='" . $value['lids_id'] . "' data-lidsLE-id='" . $value['lids_id'] . "' data-patient-id='".$patientid."' checked / >" . $value['lids_name'] . "</label>&nbsp;&nbsp;&nbsp;&nbsp";
								}
								else {
									echo "<input type='checkbox' style='width: 20px;height: 20px; vertical-align: bottom;margin-right:5px;' class='i-checks m-l get_lidsLE_prior' name='" . $value['lids_id'] . "' value='" . $value['lids_id'] . "' data-lidsLE-id='" . $value['lids_id'] . "' data-patient-id='".$patientid."'  / >" . $value['lids_name'] . "</label>&nbsp;&nbsp;&nbsp;&nbsp";
								}



								//echo "<input type='checkbox'  class='i-checks m-l get_lidsLE_prior' name='" . $value['lids_id'] . "' value='" . $value['lids_id'] . "' data-lids-id='" . $value['lids_id'] . "' data-patient-id='".$patientid."'  / >" . $value['lids_name'] . "</label>&nbsp;&nbsp;&nbsp;&nbsp";
