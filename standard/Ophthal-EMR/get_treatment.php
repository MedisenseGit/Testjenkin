...
if(isset($_GET['treatid']) && !empty($_GET['treatid'])){
	...
	} else
	{
		...
		$arrFileds[]='status';
		$arrValues[]="1";
		$checkTreat= $objQuery->mysqlSelect("dft_id","doc_patient_treatment_active","dft_id='".$treat_id."' and patient_id = '".$_GET['patientid']."' and doc_id='".$admin_id."' and doc_type='2' and status='1'","","","","");
		if(COUNT($checkTreat)==0){
		$insert_symptoms=$objQuery->mysqlInsert('doc_patient_treatment_active',$arrFileds,$arrValues);
		}
		$check_treat = $objQuery->mysqlSelect("*","doctor_frequent_treatment","dft_id='".$treat_id."' and doc_id='".$admin_id."' and doc_type='2'","","","","");
...
if(isset($_GET['edittreatid']) && !empty($_GET['edittreatid'])){
	...
	} else
	{
		...
		$arrFileds[]='status';
		$arrValues[]="0";
		$checkTreat= $objQuery->mysqlSelect("dft_id","doc_patient_treatment_active","dft_id='".$treat_id."' and patient_id = '".$patient_id."' and doc_id='".$admin_id."' and doc_type='2' and episode_id='".$_GET['episodeid']."'","","","","");
		if(COUNT($checkTreat)==0){
		$insert_symptoms=$objQuery->mysqlInsert('doc_patient_treatment_active',$arrFileds,$arrValues);
		}
		$check_treat = $objQuery->mysqlSelect("*","doctor_frequent_treatment","dft_id='".$treat_id."' and doc_id='".$admin_id."' and doc_type='2'","","","","");
					...
					if(count($check_treat)>0){
						$arrFieldsTREATFREQ[] = 'freq_count';
						$arrValuesTREATFREQ[] = $freq_count;
						$update_icd=$objQuery->mysqlUpdate('doctor_frequent_treatment',$arrFieldsTREATFREQ,$arrFieldsTREATFREQ,"dft_id = '".$check_treat[0]['dft_id']."'");
					}
...
if(isset($_GET['deltreament'])){
	...
	$objQuery->mysqlDelete('doc_patient_treatment_active',"treatment_id='".$_GET['deltreament']."'");
}

if(isset($_GET['treatid'])){
$getTreatment= $objQuery->mysqlSelect("b.treatment as treatment,a.treatment_id as treatment_id","doc_patient_treatment_active as a left join doctor_frequent_treatment as b on a.dft_id=b.dft_id","a.doc_id='".$admin_id."' and a.patient_id='".$_GET['patientid']."' and a.doc_type='2' and a.status='1'","a.treatment_id asc","","","");
...
$getTreatment= $objQuery->mysqlSelect("b.treatment as treatment,a.treatment_id as treatment_id","doc_patient_treatment_active as a left join doctor_frequent_treatment as b on a.dft_id=b.dft_id","a.episode_id='".$_GET['episodeid']."'","a.treatment_id asc","","","");
