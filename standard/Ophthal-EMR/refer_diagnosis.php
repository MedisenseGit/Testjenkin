...
if(isset($_GET['diagnoid']))
{
	...
	$getPatient= $objQuery->mysqlSelect("*","doc_my_patient","patient_id='".$_GET['patientid']."'");
	
	$getDiagno= $objQuery->mysqlSelect("*","Diagnostic_center","diagnostic_id='".$_GET['diagnoid']."'");
	$getDoc= $objQuery->mysqlSelect("*","referal","ref_id='".$admin_id."'");
	$checkDiagnoCust= $objQuery->mysqlSelect("*","diagnostic_customer","diagnostic_id='".$_GET['diagnoid']."' and patient_id='".$_GET['patientid']."'");
	//Insert 'diagnostic_customer and diagnostic_referrals table'
		...
		if(count($checkDiagnoCust)>0){
			$update_icd=$objQuery->mysqlUpdate('diagnostic_customer',$arrFileds,$arrValues,"diagnostic_id = '".$_GET['diagnoid']."' and patient_id='".$_GET['patientid']."'");
					
		}
		else{
			$insert_temp_value=$objQuery->mysqlInsert('diagnostic_customer',$arrFileds,$arrValues);
		}
	...
	//Insert 'diagnostic_referrals' table
		...
		$insert_temp_value=$objQuery->mysqlInsert('diagnostic_referrals',$arrFileds_referral,$arrValues_referral);
...
if(isset($_GET['pharmaid']))
{
	...
	$getPharma= $objQuery->mysqlSelect("*","pharma","pharma_id='".$_GET['pharmaid']."'");
	$getDoc= $objQuery->mysqlSelect("*","referal","ref_id='".$admin_id."'");
	
	
	$link = "https://medisensecrm.com/premium/Diagnostic-Refer?d=" . md5($_GET['patientid']);
...
if(isset($_GET['opticleid']))
{
	...
	$getOpticle= $objQuery->mysqlSelect("*","Opticle_center","opticale_id='".$_GET['opticleid']."'");
	$getDoc= $objQuery->mysqlSelect("*","referal","ref_id='".$admin_id."'");
