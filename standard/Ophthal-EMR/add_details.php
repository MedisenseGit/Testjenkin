...
if(!empty($_GET['frmDate']) && !empty($_GET['toDate'])){
	...
	$appointmentRange = $objQuery->mysqlSelect("*","appointment_transaction_detail","pref_doc='".$admin_id."' and Visiting_date BETWEEN '".$fromDate."' and '".$toDate."'","Visiting_date ASC","","","");
	...
	?>
	 ...
	 <input type="text" class="form-control input-sm m-b-xs" id="filter"
					 ...
					 <table class="footable table table-stripped" data-page-size="8" data-filter=#filter>
							...
							$getTimeSlot= $objQuery->mysqlSelect("*","timings","Timing_id='".$appointmentlist['Visiting_time']."'","","","","");
							?>
							<tr>
...
if(isset($_POST['update_settings'])){
	...
	$checkSetting= $objQuery->mysqlSelect("*","doctor_settings","doc_id='".$admin_id."' and doc_type='2'","","","","");
		if(count($checkSetting)>0)
		{
			$update_medicine=$objQuery->mysqlUpdate('doctor_settings',$arrField,$arrVal,"doc_id='".$admin_id."' and doc_type='2'");
		}
		else
		{
			$arrField[]="doc_id";
			...
			$arrField[]="doc_type";
			$arrVal[]='1';
			$insert_patient=$objQuery->mysqlInsert('doctor_settings',$arrField,$arrVal);
		}
		$response="Updated";
...
if(isset($_POST['addPrandialCount']))
{
		...
		$checkTrend= $objQuery->mysqlSelect("*","trend_analysis","date_added='".$getDate."' and patient_id='".$patient_id."' and patient_type='1'","","","","");
		if(count($checkTrend)>0)
		{
			$update_medicine=$objQuery->mysqlUpdate('trend_analysis',$arrFields,$arrValues,"date_added='".$getDate."' and patient_id = '".$patient_id."' and patient_type='1'");
		}
		else
		{
		$insert_patient=$objQuery->mysqlInsert('trend_analysis',$arrFields,$arrValues);
		}
		$response="updated";
...
if(isset($_POST['add_diagno']) || isset($_POST['add_diagno_patient'])){
	...
	$diagnocreate=$objQuery->mysqlInsert('doc_diagnostics',$arrFields_refer,$arrValues_refer);	
...
if(isset($_POST['add_button'])){
	$param=split("-", $_POST['patient_name']);
	...
	$transactioncreate=$objQuery->mysqlInsert('payment_transaction',$arrFields_payment,$arrValues_payment);
...
if(isset($_POST['cmdReg'])){
	...
	$getDocMail = $objQuery->mysqlSelect("b.contact_email as Contact_mail,a.company_name as OrgName","compny_tab as a left join offers_events as b on a.company_id=b.company_id","b.event_id='".$event_id."'" ,"","","","");
...
if(isset($_POST['addJobRequest'])){
	...
	$getApplicantDetails = $objQuery->mysqlSelect("a.ref_name as Doc_Name,a.ref_mail as Email_id,a.contact_num as Contact_num,b.spec_name as Specialization","referal as a left join specialization as b on a.doc_spec=b.spec_id","a.ref_id='".$admin_id."'" ,"","","","");
	$getEventName= $objQuery->mysqlSelect("title","offers_events","event_id='".$event_id."'" ,"","","","");
	$id= mysql_insert_id();
...
if($_POST['act']=="add-reschedule"){
	...
	$arrValues = array();
		...
		$patientRef=$objQuery->mysqlUpdate('appointment_transaction_detail',$arrFields,$arrValues,"appoint_trans_id='".$_POST['patTransId']."'");
		
		$checkTokenAppInfo = $objQuery->mysqlSelect("*","appointment_token_system","appoint_trans_id='".$_POST['patTransId']."'" ,"","","","");
		if(!empty($checkTokenAppInfo)){
			$getTiming = $objQuery->mysqlSelect("*","timings","Timing_id='".$slctTime."'","","","","");
			$arrFields_token[] = 'app_date';
			$arrValues_token[] = $visitDate;
			$arrFields_token[] = 'app_time';
			$arrValues_token[] = $getTiming[0]['Timing'];
		
		$patientRef=$objQuery->mysqlUpdate('appointment_token_system',$arrFields_token,$arrValues_token,"appoint_trans_id='".$_POST['patTransId']."'");
		}		
				
		$getInfo1 = $objQuery->mysqlSelect("*","appointment_transaction_detail","appoint_trans_id='".$_POST['patTransId']."'" ,"","","","");	
...
if(isset($_POST['ref_appointment'])){
	...
	$transid=time();
	$get_pro = $objQuery->mysqlSelect('*','referal as a inner join doctor_hosp as b on a.ref_id=b.doc_id inner join hosp_tab as c on c.hosp_id=b.hosp_id',"a.ref_id='".$admin_id."'");
	...
	else
	{
		...
		//Insert to new_hospvisitor_details table
			...
			//Purpose:Appointment token System
				...
				$getLastAppInfo = $objQuery->mysqlSelect("*","appointment_token_system","app_date='".date('Y-m-d',strtotime($chkInDate))."' and doc_id='".$admin_id."' and doc_type='2'" ,"token_no desc","","","");
...
if(isset($_POST['act'])){
	...
	$msg = "Hi - If you have any medical reports upload here ".$getUrl." - Thank you";
	send_msg($txtMob,$msg);
...
if(isset($_POST['cmdBlg'])){
	...
	$getDocDetails = $objQuery->mysqlSelect("a.ref_id as ref_id,b.hosp_id as hosp_id,c.company_id as company_id","referal as a left join doctor_hosp as b on a.ref_id=b.doc_id left join hosp_tab as c on c.hosp_id=b.hosp_id","a.ref_id='".$admin_id."'","","","","");
	...
	$addblogsofferlist=$objQuery->mysqlInsert('blogs_offers_events_listing',$arrFields1,$arrValues1);
	...
	$addSearch=$objQuery->mysqlInsert('blogs_offers_events_search',$arrFields_search,$arrValues_search);
...
if(isset($_POST['change_password'])){
	...
	if($txtPass==$txtRePass){
		...
		$editrecord=$objQuery->mysqlUpdate('referal',$arrFields,$arrValues,"ref_id='".$_POST['Prov_Id']."'");
...
if(isset($_POST['cmdshareinner'])){
	...
	$response="share-link-success";
		...
		header('location:https://medisensecrm.com'.$_POST['currenturl'].'&response='.$response);
	}
	else{
		$_SESSION['status']="error";
		
		header('location:https://medisensecrm.com'.$_POST['currenturl']);
	}
}	
	...
	$arrValues = array();
		...
		$visitDate=date('d M-Y',strtotime($getInfo1[0]['Visiting_date']));
			...
			//Purpose:Appointment token System
				...
				//Check Last Appointment Token No
			/*	$getLastAppInfo = $objQuery->mysqlSelect("*","appointment_token_system","app_date='".date('Y-m-d',strtotime($visitDate))."'" ,"","","","");
				if(COUNT($getLastAppInfo)>0){
					$getTokenNo = $getLastAppInfo[0]['token_no']+1;
				}
				else{
...
if(isset($_POST['cmdreassign'])){
	...
	$updatereferral=$objQuery->mysqlUpdate('patient_referal',$arrFields,$arrValues,"patient_id='".$patid."' and ref_id='".$_POST['oldrefid']."'");	

	$updateChatHistory=$objQuery->mysqlUpdate('chat_notification',$arrFields,$arrValues,"patient_id='".$patid."' and ref_id='".$_POST['oldrefid']."'");	
	
	//Get Reassigned Doctor details
	$getReassignedDoc=$objQuery->mysqlSelect("a.ref_id as ref_id,a.ref_name as ref_name,a.doc_spec as doc_spec,a.Total_Referred as Total_Referred,c.communication_status as communication_status,a.doc_photo as doc_photo,a.ref_address as ref_address,a.doc_state as doc_state,c.hosp_name as hosp_name,c.company_id as company_id","referal as a inner join doctor_hosp as b on a.ref_id=b.doc_id inner join hosp_tab as c on c.hosp_id=b.hosp_id","a.ref_id='".$SelectRef."'","","","","");	
	$getReassignedDocSpec=$objQuery->mysqlSelect("spec_name","specialization","spec_id='".$getReassignedDoc[0]['doc_spec']."'","","","","");
	...
	$getOldDoc=$objQuery->mysqlSelect("*","referal","ref_id='".$_POST['oldrefid']."'","","","","");
	...
	$updatereferral1=$objQuery->mysqlUpdate('referal',$arrFields2,$arrValues2,"ref_id='".$_POST['oldrefid']."'");	
	...
	$getPartnerInfo=$objQuery->mysqlSelect("a.partner_name as partner_name,a.cont_num1 as cont_num,a.Email_id as Email_id","our_partners as a left join source_list as b on a.partner_id=b.partner_id","b.source_id='".$_POST['sourceid']."'","","","","");
...
if(isset($_POST['addOffers']) || isset($_POST['editOffers'])){
	...
	/* Uploading image file */ 
		...
		$getrefPartlist = $objQuery->mysqlSelect("*","our_partners as a left join mapping_hosp_referrer as b on a.partner_id=b.partner_id","b.hosp_id='".$slctHosp."'","","","","");
		...
		$docid=$admin_id;
			...
			$smalimg="https://medisensecrm.com/Hospital/images/leap_push_icon.png";
		...
		$getDoclist = $objQuery->mysqlSelect("gcm_tokenid as GCM","referal","gcm_tokenid!=''","","","","");
		...
		$errorMessage="Sorry '".$chkreflist[0]['ref_name']."' referal is already existed";
			...
			echo '</script>'; 
					...
					//Doc Info EMAIL notification Sent to Patient
						...
						$mas_logo="<img src='https://medisensehealth.com/new_assets/images/mediassist-logo-new.png' alt='Medi Assist' width='78' height='62'>";
						}
						else{
							$mas_logo="";
						}
...
if(isset($_POST['add_person']) || isset($_POST['edit_person'])){
	...
	$arrValues = array();
		...
		if(isset($_POST['add_person'])){
			$personcreate=$objQuery->mysqlInsert('hosp_marketing_person',$arrFields,$arrValues);
			$person_id= mysql_insert_id();
			header("Location:Add-Marketing-Persons?response=add");
				
		}
		else{	
			$updateProvider=$objQuery->mysqlUpdate('hosp_marketing_person',$arrFields,$arrValues,"person_id='".$_POST['Person_Id']."'");	
...
if(isset($_POST['add_referrer']) || isset($_POST['edit_referrer'])){
	...
	$get_organisation = $objQuery->mysqlSelect('company_id as Comp_Id,company_name as Comp_name,mobile as Org_Contact,email_id as Comp_Email,company_logo as Logo','compny_tab',"company_id='".$_POST['CompId']."'");
	...
	$arrValues = array();
		...
		//EDIT DETAILS
			...
			$updatePartner=$objQuery->mysqlUpdate('our_partners',$arrFields,$arrValues,"partner_id='".$_POST['Partner_Id']."'");	
			$arrFields1[] = 'market_person_id';
			$arrValues1[] = $selectPerson;
			$updateMapping=$objQuery->mysqlUpdate('mapping_hosp_referrer',$arrFields1,$arrValues1,"partner_id='".$_POST['Partner_Id']."' and company_id='".$admin_id."'");
...
if(isset($_POST['add_hospital']) || isset($_POST['edit_hospital'])){
	...
	if(isset($_POST['add_hospital'])){
			$usercraete=$objQuery->mysqlInsert('hosp_tab',$arrFields,$arrValues);
			...
			foreach($_POST['slct_amenity'] as $amenkey => $amenValue ){
				...
				$insertAmenity=$objQuery->mysqlInsert('add_hosp_amenity',$arrFields2,$arrValues2);
			}
			...
			$file_type=$_FILES['file-3']['type'][$key];
				...
				$arrValues3[] = $file_name;
					...
					$bslist_pht=$objQuery->mysqlInsert('add_hosp_picture',$arrFields3,$arrValues3);
					$id= mysql_insert_id();
		...
		$updateProvider=$objQuery->mysqlUpdate('hosp_tab',$arrFields,$arrValues,"hosp_id='".$_POST['Hosp_Id']."'");	
					foreach($_POST['slct_amenity'] as $amenkey => $amenValue ){
						$arrFields2 = array();
						$arrValues2 = array();
...
if(isset($_POST['edit_doctor'])){
	...
	$updateProvider=$objQuery->mysqlUpdate('referal',$arrFields,$arrValues,"ref_id='".$_POST['Prov_Id']."'");
			$objQuery->mysqlDelete('doc_specialization',"doc_id='".$_POST['Prov_Id']."'");
					...
					$insert_spec=$objQuery->mysqlInsert('doc_specialization',$arrFields_spec,$arrValues_spec);
					}	
			$objQuery->mysqlDelete('doctor_hosp',"doc_id='".$_POST['Prov_Id']."'");
					...
					$insert_spec=$objQuery->mysqlInsert('doctor_hosp',$arrFields_hosp,$arrValues_hosp);
					}
...
$objQuery->mysqlDelete('doc_time_set',"doc_id='".$_POST['Prov_Id']."'");
	...
	for($i=1; $i<=$_POST['limit_i']; $i++)
	{
		...
		for($j=1; $j<=$_POST['limit_j']; $j++)
		{
			...
			$doctimecreate=$objQuery->mysqlInsert('doc_time_set',$arrFields_time,$arrValues_time);
...
if(isset($_POST['add_amenity']) || isset($_POST['edit_amenity'])){
	...
	$chkHospPicture= $objQuery->mysqlSelect("*","add_hosp_picture","hosp_id='".$_POST['slctHosp']."'","","","","");
...
if(isset($_POST['addActivity']) && !empty($_POST['txtDesc'])){
					...
					if($getStatus2<5){  //Status2 will change only when present status remains in below respond level, ie. it must be in 'New'/Refered/P-Awating Status
						...
						$getRef = $objQuery->mysqlSelect("*","referal as a left join doctor_hosp as b on a.ref_id=b.doc_id left join hosp_tab as c on c.hosp_id=b.hosp_id","a.ref_id='".$_POST['doc_id']."'","","","","");
						...
						$arrValues3[]= $TotCount;
						$updateCount=$objQuery->mysqlUpdate('referal',$arrFields3,$arrValues3,"ref_id='".$_POST['doc_id']."'");
						
						//Update response time 
						//RETREIVE DOCTOR'S FIRST REFERRED DATE
						$getDocResponse = $objQuery->mysqlSelect("TImestamp as Chat_Date,chat_note as Chat_Note","chat_notification","patient_id='".$_POST['patient_id']."'and ref_id='".$_POST['doc_id']."'","","","","");
						...
						$arrValues2[]= "5";
						$updateBucket=$objQuery->mysqlUpdate('patient_referal',$arrFields2,$arrValues2,"patient_id='".$_POST['patient_id']."'");
					...
					$getPartnerRespSetting = $objQuery->mysqlSelect("*","our_partners as a left join source_list as b on a.partner_id=b.partner_id","b.source_id='".$getPatInfo[0]['patient_src']."'","","","","");
					
					$getMarketPerson = $objQuery->mysqlSelect("*","hosp_marketing_person as a left join mapping_hosp_referrer as b on a.person_id=b.market_person_id","b.partner_id='".$getPartnerRespSetting[0]['partner_id']."' and b.hosp_id='".$getPatInfo[0]['hosp_id']."'","","","","");
