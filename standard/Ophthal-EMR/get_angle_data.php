...
if(isset($_GET['angleid']) && !empty($_GET['angleid'])){
	...
	if(is_numeric($params[0]) == false){
		...
		$check_exists_angle = $objQuery->mysqlSelect("*","examination_ophthal_angle","angle_name='".$params[0]."' and doc_id='".$admin_id."' and doc_type='2' ","","","","");
		if(COUNT($check_exists_angle)==0){ //To prevent double entry
			$insert_angle=$objQuery->mysqlInsert('examination_ophthal_angle',$arrFileds_master,$arrValues_master);
			$angle_id = mysql_insert_id(); //Get Patient Id
			
			$arrFileds[]='angle';
			$arrValues[]=$angle_id;
			$insert_chamber=$objQuery->mysqlInsert('doc_patient_angle_active',$arrFileds,$arrValues);
	...
	} else
	{
		...
		if(isset($_GET['eye_type']) && isset($_GET['angle_status'])) {
		    ...
		    $upd_angle = $objQuery->mysqlUpdate("examination_ophthal_angle", array($upd_field), array($upd_val), $upd_where);
			
			if($upd_val == 0) {
				if($Edit_Session == 1){
				$objQuery->mysqlDelete('doc_patient_angle_active',"episode_id='".$Episode_Id."' and angle='".$angle_id."' and doc_id='".$admin_id."' and doc_type='2' and eye_side='".$_GET['eye_type']."' and patient_id='".$_GET['patientid']."' and status='0'");
				}
				else
				{
				$objQuery->mysqlDelete('doc_patient_angle_active',"angle='".$angle_id."' and doc_id='".$admin_id."' and doc_type='2' and eye_side='".$_GET['eye_type']."' and patient_id='".$_GET['patientid']."' and status='1'");
				}
			}
			else {
