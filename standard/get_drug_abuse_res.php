		...
		$insert_drug=$objQuery->mysqlInsert('doc_patient_drug_active',$arrFileds,$arrValues);
				...
				$check_drug = $objQuery->mysqlSelect("*","doctor_frequent_drug_abuse","drug_abuse_id='".$drug_id."' and doc_id='".$admin_id."' and doc_type='1'","","","","");
					...
					if(count($check_drug)>0){
						...
						$update_icd=$objQuery->mysqlUpdate('doctor_frequent_drug_abuse',$arrFieldsSYMPFREQ,$arrValuesSYMPFREQ,"fda_id = '".$check_drug[0]['fda_id']."'");
					}
					else{
						...
						$arrFieldsSYMPFREQ[] = 'freq_count';
						$arrValuesSYMPFREQ[] = "1";
						$insert_freq_symp=$objQuery->mysqlInsert('doctor_frequent_drug_abuse',$arrFieldsSYMPFREQ,$arrValuesSYMPFREQ);
...
if(isset($_GET['deldrugid']))
{
	...
	$objQuery->mysqlDelete('doc_patient_drug_active',"drug_active_id='".$_GET['deldrugid']."'");
}

$getDrugRes= $objQuery->mysqlSelect("b.drug_abuse as drug_abuse,a.drug_active_id as drug_active_id","doc_patient_drug_active as a left join drug_abuse_auto as b on a.drug_abuse_id=b.drug_abuse_id","a.doc_id='".$admin_id."' and a.patient_id='".$patientid."' and a.doc_type='1' and a.status='1'","","","","");
